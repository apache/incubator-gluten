...
public final class ColumnarBatches {
  ...
  public static long getNativeHandle(ColumnarBatch batch) {
    if (isZeroColumnBatch(batch)) {
      // Use the internal Arrow-based backend to find an apposite native batch for the input
      // zero-column batch then return its handle. This operation will cause some empty-schema
      // native Arrow batches mixed into backend batches before sending to native for further
      // processing. The user native code is expected to handle this case correctly.
