...
class ClickHouseTestSettings extends BackendTestSettings {
  ...
  enableSuite[GlutenSubquerySuite]
    ...
    // ClickHouse backend pull out pre-project in columnar, its subquery's size
    // is same as vanilla Spark.
    .exclude(GLUTEN_TEST + "Merge non-correlated scalar subqueries in a subquery")
    .exclude(GLUTEN_TEST + "Merge non-correlated scalar subqueries from different levels")
