...
//      Row(new java.math.BigDecimal(2), new java.math.BigDecimal(6)) :: Nil)
  ...
  test(GLUTEN_TEST + "extend with cast expression") {
    checkAnswer(
      decimalData.agg(
        sum($"a".cast("double")),
        avg($"b".cast("double")),
  ...
  test(GLUTEN_TEST + "use gluten hash agg to replace vanilla spark sort agg") {
