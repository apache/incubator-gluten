    ...
    withAllParquetReaders {
      withSQLConf(SQLConf.PARQUET_FILTER_PUSHDOWN_ENABLED.key -> "true",
        ...
        withTempPath { dir =>
          ...
          (1 to 3).map(i => (i, i.toString, null: String))
            .toDF("a", "b", "c").write.parquet(path1)
          val path2 = s"${dir.getCanonicalPath}/table2"
          (1 to 3).map(i => (null: Integer, i.toString, i.toString))
            .toDF("a", "b", "c").write.parquet(path2)
...
class GlutenParquetV2FilterSuite extends GltuenParquetFilterSuite with GlutenSQLTestsBaseTrait {
  ...
  test(GlutenTestConstants.GLUTEN_TEST + "filter pushdown - date") {
    ...
    Seq(false, true).foreach { java8Api =>
      Seq(CORRECTED).foreach { rebaseMode =>
        withSQLConf(
