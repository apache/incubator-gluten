...
class GlutenHiveSQLQuerySuite extends GlutenSQLTestsTrait {
  ...
  test("cast timestamp to string") {
    sql("drop table if exists ts_tbl")
    sql("CREATE TABLE ts_tbl(ts TIMESTAMP) USING hive OPTIONS(fileFormat 'parquet')")
    sql("insert into ts_tbl values(timestamp'2019-09-06 17:15:15')")
    val data = sql("select cast(ts as string) from ts_tbl")
    checkAnswer(data, Seq(Row("2019-09-07T00:00:00.915000")))
    checkOperatorMatch[ProjectExecTransformer](data)
  }
}
