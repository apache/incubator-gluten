...
class SparkFunctionStatistics extends QueryTest {
  ...
  test(GlutenTestConstants.GLUTEN_TEST + "Run spark function statistics: ") {
    ...
    Seq("array_contains", "map_keys", "get_json_object", "element_at", "map_from_arrays",
      "contains", "startswith", "endswith", "map_contains_key", "map_values", "try_element_at",
      "struct", "array", "ilike", "sec", "csc").foreach(
      name => {
