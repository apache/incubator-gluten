...
class GlutenKeyGroupedPartitioningSuite
  ...
  testGluten("SPARK-41471: shuffle one side: only one side reports partitioning with two identity") {
  ...
  testGluten("SPARK-44647: SPJ: test join key is subset of cluster key " +
  ...
  testGluten("SPARK-47094: Compatible buckets does not support SPJ with " +
  ...
  testGluten("SPARK-47094: SPJ: Does not trigger when incompatible number of buckets on both side") {
    val table1 = "tab1e1"
    val table2 = "table2"
