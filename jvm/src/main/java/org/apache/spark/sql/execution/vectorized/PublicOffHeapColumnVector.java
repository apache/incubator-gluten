...
/**
  * Copy from OffHeapColumnVector of Spark,
  * only modify some private field to public.
  */
public final class PublicOffHeapColumnVector extends WritableColumnVector {

  private static final boolean bigEndianPlatform =
      ByteOrder.nativeOrder().equals(ByteOrder.BIG_ENDIAN);

  public static PublicOffHeapColumnVector[] allocateColumns(int capacity, StructType schema) {
    return allocateColumns(capacity, schema.fields());
  }

  public static PublicOffHeapColumnVector[] allocateColumns(int capacity, StructField[] fields) {
    PublicOffHeapColumnVector[] vectors = new PublicOffHeapColumnVector[fields.length];
    for (int i = 0; i < fields.length; i++) {
      vectors[i] = new PublicOffHeapColumnVector(capacity, fields[i].dataType());
    }
    return vectors;
  }
  ...
  public PublicOffHeapColumnVector(int capacity, DataType type) {
    super(capacity, type);
  ...
  @Override
  protected PublicOffHeapColumnVector reserveNewColumn(int capacity, DataType type) {
    return new PublicOffHeapColumnVector(capacity, type);
  }
}
