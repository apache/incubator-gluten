...
public final class NativeColumnVector extends WritableColumnVector {

  private static final boolean bigEndianPlatform =
      ByteOrder.nativeOrder().equals(ByteOrder.BIG_ENDIAN);

  public static NativeColumnVector[] allocateColumns(int capacity, StructType schema) {
    return allocateColumns(capacity, schema.fields());
  }

  public static NativeColumnVector[] allocateColumns(int capacity, StructField[] fields) {
    NativeColumnVector[] vectors = new NativeColumnVector[fields.length];
    // create native writer
    NativeColumnVectorWriter nativeWriter =
        BackendsApiManager$.MODULE$.getIteratorApiInstance().genNativeColumnVectorWriter();
    for (int i = 0; i < fields.length; i++) {
      vectors[i] = new NativeColumnVector(capacity, fields[i].dataType(), nativeWriter);
    }
    return vectors;
  }

  private NativeColumnVectorWriter nativeWriter;

  public void setNativeWriter(NativeColumnVectorWriter nativeWriter) {
  ...
  public NativeColumnVector(int capacity, DataType type, NativeColumnVectorWriter nativeWriter) {
    super(capacity, type);
    this.type = type;
  ...
  public long valuesNativeAddress() {
    return this.nativeWriter.getNativeAddress();
  }
  ...
  public WritableColumnVector[] getChildColumns() {
    return childColumns;
  }
  ...
  @Override
  protected NativeColumnVector reserveNewColumn(int capacity, DataType type) {
    return new NativeColumnVector(capacity, type, this.nativeWriter);
  }
}
