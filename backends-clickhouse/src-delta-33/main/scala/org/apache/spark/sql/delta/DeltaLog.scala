 ...
 *
 * This file is copied from Delta 3.3.1, it is modified to overcome the following issues:
...
class DeltaLog private(
  ...
  def startTransaction(
    ...
    TransactionExecutionObserver.getObserver.startingTransaction {
      // --- modified start
      new ClickhouseOptimisticTransaction(this, catalogTableOpt, snapshotOpt)
      // --- modified end
    }
  }
  ...
  // --- modified start
  override def fileFormat(protocol: Protocol, metadata: Metadata): FileFormat = {
    if (ClickHouseConfig.isMergeTreeFormatEngine(metadata.configuration)) {
      ClickHouseTableV2.getTable(this).getFileFormat(protocol, metadata)
    } else {
      super.fileFormat(protocol, metadata)
    }
  }
  // --- modified end
}
