...
class GlutenClickHouseTPCHNewParquetSuite extends GlutenClickHouseTPCHAbstractSuite {
  ...
  override protected def createTPCHNullableTables(): Unit = {
    ...
    val saltedTablesPath = tablesPath + "-salted2"
          ...
          tableName => {
            ...
            for (c <- df.schema) {
              salted_df = Some((salted_df match {
                case Some(x) => x
                case None => df
              }).withColumn(c.name, when(rand() < 0, null).otherwise(col(c.name))))
            }
