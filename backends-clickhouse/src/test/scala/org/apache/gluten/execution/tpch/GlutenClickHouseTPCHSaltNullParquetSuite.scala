...
class GlutenClickHouseTPCHSaltNullParquetSuite extends GlutenClickHouseTPCHAbstractSuite {
  ...
  test("GLUTEN-9032 nothing types nullability") {
    val sql1 =
      """
        |select a, b, c from (
        |  select
    ...
    val sql6 =
      """
        |select a, b, c from (
        |  select
        |    n_regionkey as a, n_nationkey as b, cast(array() as String) as c
        |  from nation where n_nationkey % 2 = 1
        |  union all
        |  select n_nationkey as a, n_regionkey as b, '123' as c
        |  from nation where n_nationkey % 2 = 0
        |)
      """.stripMargin
    ...
    val sql7 =
      """
        |select a, b, c from (
        |  select
        |    n_regionkey as a, n_nationkey as b, cast(array(null) as String) as c
        |  from nation where n_nationkey % 2 = 1
        |  union all
        |  select n_nationkey as a, n_regionkey as b, '123' as c
        |  from nation where n_nationkey % 2 = 0
