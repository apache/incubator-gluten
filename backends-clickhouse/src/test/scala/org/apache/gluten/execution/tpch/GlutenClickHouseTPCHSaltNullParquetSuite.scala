...
class GlutenClickHouseTPCHSaltNullParquetSuite extends GlutenClickHouseTPCHAbstractSuite {
  ...
  test("GLUTEN-8343: Cast number to decimal") {
    val table_create_sql = "create table test_tbl_8343(id bigint, d bigint, f double) using parquet"
    val insert_data_sql =
      "insert into test_tbl_8343 values(1, 55, 55.12345), (2, 137438953483, 137438953483.12345)"
    val query_sql =
      "select cast(d as decimal(1, 0)), cast(d as decimal(9, 1)), cast(f as decimal(32, 3)), cast(f as decimal(2, 1)) from test_tbl_8343"
    spark.sql(table_create_sql);
    spark.sql(insert_data_sql);
    compareResultsAgainstVanillaSpark(query_sql, true, { _ => })
    spark.sql("drop table test_tbl_8343")
