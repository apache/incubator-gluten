...
import org.apache.spark.sql.{Row, TestUtil}
...
class GlutenClickHouseTPCHSuite extends GlutenClickHouseTPCHAbstractSuite {
  ...
  test("test 'order by'") {
    ...
    TestUtil.compareAnswers(result, expected)
  }
  ...
  test("test 'order by limit'") {
    runSql("""
      ...
      df =>
        ...
        TestUtil.compareAnswers(result, expectedResult)
    }
  }
  ...
  test("test decimal128") {
    ...
    TestUtil.compareAnswers(df2.select("b").collect(), Seq(Row(struct)))
    TestUtil.compareAnswers(
