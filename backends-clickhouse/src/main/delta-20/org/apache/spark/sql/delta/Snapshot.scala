...
class Snapshot(
  ...
  private def stateReconstruction: Dataset[SingleAction] = {
    withDmqTag {
      recordFrameProfile("Delta", "snapshot.stateReconstruction") {
        ...
        val canonicalizePath = DeltaUDF.stringStringUdf(
          (filePath: String) => Snapshot.canonicalizePath(filePath, hadoopConf.value.value))
        ...
        loadActions
          .withColumn(
            ...
            when(col("add.path").isNotNull, canonicalizePath(col("add.path"))))
          .withColumn(
            REMOVE_PATH_CANONICAL_COL_NAME,
            when(col("remove.path").isNotNull, canonicalizePath(col("remove.path"))))
          .repartition(
            getNumPartitions,
  ...
  protected def indexToRelation(
    ...
    val fsRelation =
      HadoopFsRelation(index, index.partitionSchema, schema, None, index.format, deltaLog.options)(
        spark)
