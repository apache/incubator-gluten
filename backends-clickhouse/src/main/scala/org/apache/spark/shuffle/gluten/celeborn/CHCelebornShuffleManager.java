...
public class CHCelebornShuffleManager implements ShuffleManager {

  private static final Logger logger = LoggerFactory.getLogger(CHCelebornShuffleManager.class);
  ...
  public CHCelebornShuffleManager(SparkConf conf) {
    ...
    try {
      if (handle instanceof CelebornShuffleHandle) {
        ...
        if (ShuffleMode.HASH.equals(celebornConf.shuffleWriterMode())) {
          return new CelebornHashBasedCHColumnarShuffleWriter<>(
