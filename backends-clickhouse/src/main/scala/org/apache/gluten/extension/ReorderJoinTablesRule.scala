...
case class ReorderJoinTablesRule(session: SparkSession) extends Rule[SparkPlan] with Logging {
  ...
  private def reorderHashJoin(hashJoin: CHShuffledHashJoinExecTransformer): SparkPlan = {
    ...
    if (leftQueryStageRow == None || rightQueryStageRow == None) {
      logInfo(s"Cannot reorder this hash join. Its children is not ShuffleQueryStageExec")
      hashJoin
    } else {
      val threshold = CHBackendSettings.reorderHashJoinTablesThreadhold
      ...
      hashJoin.joinType match {
        ...
        case LeftOuter =>
          // left outer + build right is the common case，ther cases have not been covered by tests
        ...
        case RightOuter =>
          // right outer + build left is the common case，ther cases have not been covered by tests
          // and don't reroder them.
