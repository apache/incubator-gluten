...
//
// To be deprecated: Use TakeOrderedAndProjectExecTransformer#offload instead.
case class CHTakeOrderedAndProjectExecTransformer(
    limit: Long,
  ...
  override def simpleString(maxFields: Int): String = {
    ...
    s"CHTakeOrderedAndProjectExecTransformer (limit=$limit, " +
...
object CHTakeOrderedAndProjectExecTransformer {
  def transformer(): TakeOrderedAndProjectExecTransformer = {
    new TakeOrderedAndProjectExecTransformer {
      override def validate(from: TakeOrderedAndProjectExec): ValidationResult = {
        offload0(from).doValidate()
      }

      override def offload(from: TakeOrderedAndProjectExec): SparkPlan = {
        offload0(from)
      }

      private def offload0(from: TakeOrderedAndProjectExec): GlutenPlan = {
        val (limit, offset) = SparkShimLoader.getSparkShims.getLimitAndOffsetFromTopK(from)
        val t = CHTakeOrderedAndProjectExecTransformer(
          limit,
          from.sortOrder,
          from.projectList,
          from.child,
