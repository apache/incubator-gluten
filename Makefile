...
# conan package info
BUILD_VERSION ?= master # BUILD_VERSION ?= $(shell date '+%Y.%m.%d.00')
...
spark-ut-test:
	cd ep/ && rm -rf spark && git clone --branch branch-3.2-bd --depth 1  git@code.byted.org:bytedance/spark.git
	cd ..
	export GLOG_minloglevel=2 && $(call SET_JAVA_HOME,8) && java -version && \
	mvn clean install -Pspark-3.2 -Pspark-ut -Pbackends-bolt -Ppaimon -Pceleborn -DargLine="-Dspark.test.home=${ROOT_DIR}/ep/spark" -DtagsToExclude=org.apache.spark.tags.ExtendedSQLTest,io.glutenproject.tags.UDFTest,io.glutenproject.tags.SkipTestTags -Denforcer.skip=true

test_las-spark32:
	cd ep/ && rm -rf spark && git clone --branch las-release/2023q4 --depth 1  git@code.byted.org:bytedance/spark.git
	cd ..
	export GLOG_minloglevel=2 && $(call SET_JAVA_HOME,8) && java -version && \
	mvn clean install -Pspark-3.2-las -Pspark-ut -Pbackends-bolt -Ppaimon -Pceleborn -DargLine="-Dspark.test.home=${ROOT_DIR}/ep/spark" -DtagsToExclude=org.apache.spark.tags.ExtendedSQLTest,io.glutenproject.tags.UDFTest,io.glutenproject.tags.SkipTestTags -Denforcer.skip=true

spark-ut-test-jdk17:
	cd ep/ && rm -rf spark && git clone --branch branch-3.2-bd --depth 1  git@code.byted.org:bytedance/spark.git
	cd ..
	export GLOG_minloglevel=2 && \
	$(call SET_JAVA_HOME,17) && java -version && export SPARK_HOME=${ROOT_DIR}/ep/spark && \
	mvn clean install -Pjava-17 -Pspark-3.2 -Pceleborn -Pspark-ut -Pbackends-bolt -Ppaimon

spark-ut-slow-test:
	cd ep/ && rm -rf spark && git clone --branch branch-3.2-bd --depth 1  git@code.byted.org:bytedance/spark.git
	cd .. 
	export GLOG_minloglevel=2 && $(call SET_JAVA_HOME,8) && java -version && mvn clean install -Pspark-3.2 -Pspark-ut -Pbackends-bolt -Ppaimon -Pceleborn -DargLine="-Dspark.test.home=${ROOT_DIR}/ep/spark" -DtagsToInclude=org.apache.spark.tags.ExtendedSQLTest -Denforcer.skip=true
