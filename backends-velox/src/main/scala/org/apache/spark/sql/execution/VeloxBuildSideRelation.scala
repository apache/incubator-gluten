...
case class VeloxBuildSideRelation(
  ...
  override def asReadOnlyCopy(broadCastContext: BroadCastHashJoinContext): VeloxBuildSideRelation =
    this
  ...
  override def transform(key: Expression): Array[InternalRow] = {
    ...
    val batchIter = ArrowConverterUtils.convertFromNetty(output, batches)
      ...
      .flatMap(
        batch => {
          if (batch.numRows == 0) {
            ...
            this.output.isEmpty || (batch.numCols() > 0 &&
