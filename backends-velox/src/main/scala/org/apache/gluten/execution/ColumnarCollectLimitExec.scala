...
case class ColumnarCollectLimitExec(
  ...
  private def collectWithOffsetAndLimit(
    ...
    var rowsToSkip = math.max(offset, 0) // 5
    var rowsToCollect = if (unlimited) Int.MaxValue else limit // 10
...
//        var rowsToCollect = if (unlimited) Int.MaxValue else limit // 10
        ...
        while (inputIter.hasNext) {
          ...
          if (rowsToSkip >= batchSize) {
            ...
            val prunedBatch = VeloxColumnarBatches.slice(batch, startIndex, needed)
            ColumnarBatches.retain(prunedBatch)
