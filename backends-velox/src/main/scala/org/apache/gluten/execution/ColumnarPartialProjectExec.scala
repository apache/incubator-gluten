  ...
  def create(original: SparkPlan): UnaryTransformSupport = {
    val transformedPlan = original match {
      case p: ProjectExec =>
        ...
        val partialProject =
          ColumnarPartialProjectExec(p, p.projectList, p.child)(replacedAliasUdf.toSeq)
      ...
      case f: FilterExec =>
        ...
        val newCondition = splitConjunctivePredicates(f.condition)
          .map(
            p => {
              if (containsUDF(p)) {
                replaceByAlias(p, replacedAliasUdf)
              } else p
            })
          .reduceLeftOption(And)
          .orNull
        val partialProject =
          ColumnarPartialProjectExec(f, f.output, f.child)(replacedAliasUdf.toSeq)
