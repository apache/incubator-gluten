...
class TestOperator extends WholeStageTransformerSuite {
  ...
  test("test_where") {
    ...
    assert(df.collect().length == 59288)
  }
  ...
  test("test_in_not") {
    ...
    assert(df.collect().length == 60141)
  }
  ...
  test("Test chr function") {
    ...
    assert(df.queryExecution.executedPlan
      .find(_.isInstanceOf[ProjectExecTransformer]).isDefined)
  }
