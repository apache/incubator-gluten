...
class TestOperator extends WholeStageTransformerSuite {
    ...
    test("select_part_column") {
        val result = runSql("select l_shipdate, l_orderkey from lineitem limit 1")
        { df => { assert(df.schema.fields.length == 2)}}
        assert(result.length == 1)
    }

    test("select_as") {
        val result = runSql("select l_shipdate as my_col from lineitem limit 1")
    ...
    test("test_is_null_has_null") {
        ...
        spark.createDataFrame(JavaConverters.seqAsJavaList(data), schema)
    ...
    test("test_is_not_null") {
        val result = runSql("select l_orderkey from lineitem where l_comment is not null " +
          "and l_orderkey = 1") { _ => }
    ...
    ignore("test_and pushdown") {
        val result = runSql("select l_orderkey from lineitem where l_orderkey > 2 " +
          "and l_orderkey = 1") { _ => }
        assert(result.isEmpty)
    }
    ...
    ignore("test_in_and") {
        val result = runSql("select l_orderkey from lineitem " +
          "where l_partkey in (1552, 674, 1062) and l_partkey in (1552, 674)")
        { _ => }
    ...
    test("test_in_or") {
        val result = runSql("select l_orderkey from lineitem " +
          "where l_partkey in (1552, 674) or l_partkey in (1552, 1062)")
        { _ => }
        assert(result.length == 122)
    }
    ...
    test("test_in_or_and") {
        val result = runSql("select l_orderkey from lineitem " +
          "where l_partkey in (1552, 674) or l_partkey in (1552) and l_orderkey > 1")
        { _ => }
        assert(result.length == 73)
    }
    ...
    test("test_in_not") {
        val result = runSql("select l_orderkey from lineitem " +
          "where l_partkey not in (1552, 674) or l_partkey in (1552, 1062)")
        { _ => }
    ...
    test("test_avg") {
        val result = runSql("select avg(l_partkey) from lineitem where l_partkey < 1000")
        { _ => }
        assert(result.length == 1)
    }

    test("test_sum") {
        val result = runSql("select sum(l_partkey) from lineitem where l_partkey < 2000")
        { _ => }
        assert(result.length == 1)
    }

    test("test_groupby") {
        val result = runSql("select l_orderkey, sum(l_partkey) as sum from lineitem " +
    ...
    test("test_orderby") {
        val result = runSql("select l_suppkey from lineitem " +
          "where l_orderkey < 3 order by l_partkey")
        { _ => }
