...
public class TestParquetScan extends AvroDataTest {
  ...
  private boolean containsMapType(Type type) {
    switch (type.typeId()) {
        case MAP -> {
            return true;
        }
        case LIST -> {
          return containsMapType(type.asListType().elementType());
        }
        case STRUCT -> {
          for (final Types.NestedField child : type.asStructType().fields()) {
            return containsMapType(child.type());
          }
        }
    }
    return false;
  ...
  protected void writeAndValidate(Schema schema) throws IOException {
    ...
    // Map type is not supported to sort.
    configureVectorization(table);
    List<String> sortCols = schema.columns().stream()
            .filter(c -> !containsMapType(c.type()))
            .map(c -> c.name()).collect(Collectors.toList());
    Dataset<Row> df = spark.read().format("iceberg").load(table.location());
    if (!sortCols.isEmpty()) {
      List<String> otherCols = sortCols.stream().collect(Collectors.toList());
      otherCols.remove(0);
      df = df.orderBy(sortCols.get(0), JavaConverters.asScalaBufferConverter(otherCols).asScala().toSeq());
    }
    List<Row> rows = df.collectAsList();
    spark.conf().set("spark.gluten.enabled", "false");
    Dataset<Row> df2 = spark.read().format("iceberg").load(table.location());
    if (!sortCols.isEmpty()) {
      List<String> otherCols = sortCols.stream().collect(Collectors.toList());
      otherCols.remove(0);
      df2 = df2.orderBy(sortCols.get(0), JavaConverters.asScalaBufferConverter(otherCols).asScala().toSeq());
    }
    List<Row> rows2 = df2.collectAsList();
    for (int i = 0; i < rows2.size(); i++) {
      Assert.assertEquals(rows.get(i), rows2.get(i));
    }

    Assert.assertEquals(rows, df2.collectAsList());
