  ...
  public static TypedExpr toTypedExpr(RexNode rexNode, List<String> inNames) {
    if (rexNode instanceof RexLiteral) {
      ...
      return new CallTypedExpr(
          nodeType, params, FunctionMappings.toVeloxFunction(rexCall.getOperator().getName()));
    } else if (rexNode instanceof RexInputRef) {
      RexInputRef inputRef = (RexInputRef) rexNode;
      return FieldAccessTypedExpr.create(
          toType(inputRef.getType()), inNames.get(inputRef.getIndex()));
    } else if (rexNode instanceof RexFieldAccess) {
      RexFieldAccess fieldAccess = (RexFieldAccess) rexNode;
      return FieldAccessTypedExpr.create(
          toTypedExpr(fieldAccess.getReferenceExpr(), inNames), fieldAccess.getField().getName());
    } else {
