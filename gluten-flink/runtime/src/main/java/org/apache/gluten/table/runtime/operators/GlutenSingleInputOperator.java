...
public class GlutenSingleInputOperator extends TableStreamOperator<RowData>
  ...
  public void open() throws Exception {
    ...
    List<BoundSplit> splits =
        List.of(
            new BoundSplit(
                id, -1, new ExternalStreamConnectorSplit("connector-external-stream", es.id())));
  ...
  public void processElement(StreamRecord<RowData> element) {
    final RowVector inRv =
        FlinkRowToVLVectorConvertor.fromRowData(element.getValue(), allocator, session, inputType);
    inputQueue.add(inRv);
