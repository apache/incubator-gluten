...
public class GlutenKafkaDynamicSource extends KafkaDynamicSource {
  ...
  public GlutenKafkaDynamicSource(
  ...
  public static GlutenKafkaDynamicSource copyFrom(KafkaDynamicSource source) throws Exception {
    ...
    // copy.watermarkStrategy = getFieldFromKafkaSource(source, "watermarkStrategy",
  ...
  private static <T> T getFieldValue(
      Object source, Class<?> sourceClazz, String fieldName, Class<T> fieldClazz) {
    try {
      ...
      if (fClazz.equals(fieldClazz)) {
        ...
        String errMsg =
            String.format(
                "Faield to get field %s from KafkaDynamimcSource,  the field class {} not match with given class: {}",
  ...
  private static <T> T getFieldFromKafkaSource(
      KafkaDynamicSource source, String fieldName, Class<T> fieldClazz) {
    return getFieldValue(source, KafkaDynamicSource.class, fieldName, fieldClazz);
  }

  private static <T> T getFieldFromKafkaSource(
      KafkaSource<?> source, String fieldName, Class<T> fieldClazz) {
    return getFieldValue(source, KafkaSource.class, fieldName, fieldClazz);
  }

  private static <T> T invokeMethodFromKafkaSource(
      KafkaDynamicSource source, String methodName, Class<?>[] argClazzs, Object[] args) {
    try {
  ...
  private static String getFormat(DeserializationSchema<RowData> deserializationSchema) {
    ...
    if (deserializationSchema instanceof RawFormatDeserializationSchema) {
      ...
      throw new RuntimeException(
          "Format not supported:" + deserializationSchema.getClass().getName());
    }
  }
  ...
  public ScanRuntimeProvider getScanRuntimeProvider(ScanContext context) {
    final DeserializationSchema<RowData> keyDeserialization =
        invokeMethodFromKafkaSource(
            this,
            "createDeserialization",
            new Class<?>[] {
              DynamicTableSource.Context.class, DecodingFormat.class, int[].class, String.class
            },
    ...
    final KafkaRecordDeserializationSchema<RowData> deserializationSchema =
        KafkaRecordDeserializationSchema.of(
            invokeMethodFromKafkaSource(
                ...
                new Class<?>[] {
                  DeserializationSchema.class, DeserializationSchema.class, TypeInformation.class
                },
    ...
    KafkaSubscriber subscriber =
        getFieldFromKafkaSource(kafkaSource, "subscriber", KafkaSubscriber.class);
    OffsetsInitializer startOffsetsInitializer =
        getFieldFromKafkaSource(
            kafkaSource, "startingOffsetsInitializer", OffsetsInitializer.class);
    OffsetsInitializer stopOffsetsInitializer =
        getFieldFromKafkaSource(
            kafkaSource, "stoppingOffsetsInitializer", OffsetsInitializer.class);
    ...
    return new DataStreamScanProvider() {
      ...
      public DataStream<RowData> produceDataStream(
        ...
        DataStreamSource<RowData> sourceStream =
            execEnv.fromSource(
                glutenKafkaSource, watermarkStrategy, "kafkasource-" + tableIdentifier);
        glutenKafkaSource.setPlanNodeId(PlanNodeIdGenerator.newId());
        glutenKafkaSource.setEnableAutoCommitOffset(
            !execEnv.getCheckpointConfig().isCheckpointingEnabled());
