  ...
  object ConfImplicits {
    ...
    implicit class SparkConfWrapper(conf: SparkConf) {
      ...
      def setWarningOnOverriding(key: String, value: String): SparkConf = {
        setConfSafe(key, value, onOverriding => {
          Console.err.println(s"Overriding SparkConf key ${onOverriding.key}, old value: ${onOverriding.value}, new value: ${onOverriding.newValue}. ")
        })
      }
      ...
      private def setConfSafe(key: String, value: String, onOverriding: OnOverriding => Unit): SparkConf = synchronized {
