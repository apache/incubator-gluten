...
Otherwise, you must add a new package in `./ports/$package` to override the vcpkg version.
...
Some library not support out-of-tree build. Here is a hidden option for `vcpkg_configure_make()`:
...
Vcpkg will also post-validate package and warn common errors.
