...
function concat_velox_param {
    ...
    if [ "$ENABLE_ENHANCED_FEATURE" = "ON" ]; then
        VELOX_PARAMETER+="--enable_enhanced_feature=$ENABLE_ENHANCED_FEATURE "
    fi
...
function build_gluten_cpp {
  ...
  GLUTEN_CMAKE_OPTIONS="-DBUILD_VELOX_BACKEND=ON \
    ...
    -DENABLE_ENHANCED_FEATURE=$ENABLE_ENHANCED_FEATURE"
