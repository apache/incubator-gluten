...
def select_compilation_db(compilation_db, selection):
    ...
    for path in selection:
        ...
        if not found:
            print(
                "warning: '%s' not found in compilation database." % path,
                file=sys.stderr,
            )
            continue
...
def slice_compilation_db(compilation_db, selection, exclude):
    ...
    for path in exclude:
        if not os.path.exists(path):
            print(
                "warning: excluded path '%s' not found on disk." % path, file=sys.stderr
            )
            continue
...
def main(
    ...
    if not IWYU_EXECUTABLE:
        print("error: include-what-you-use executable not found", file=sys.stderr)
        return 1
    ...
    try:
        ...
        print("error: failed to parse compilation database: %s" % why, file=sys.stderr)
        return 1
...
def _bootstrap(sys_argv):
    ...
    parser.add_argument(
        ...
        const=0,
        help=(
            "Number of concurrent subprocesses. If zero, "
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Do not start new jobs if the 1min load average "
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help=("Do not run IWYU on source files (or directories) " "below this path."),
    )
    parser.add_argument(
        "source",
        nargs="*",
        help=(
            "Zero or more source files (or directories) to "
