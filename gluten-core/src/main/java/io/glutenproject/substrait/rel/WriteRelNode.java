...
public class WriteRelNode implements RelNode, Serializable {
  ...
  private final ArrayList<ColumnTypeNode> partitionColumnTypeNodes = new ArrayList<>();

  private final AdvancedExtensionNode extensionNode;

  WriteRelNode(RelNode input, ArrayList<TypeNode> types, ArrayList<String> names,
               ArrayList<ColumnTypeNode> partitionColumnTypeNodes,
               String writePath,
               AdvancedExtensionNode extensionNode) {
    this.input = input;
    this.types.addAll(types);
    this.names.addAll(names);
    this.partitionColumnTypeNodes.addAll(partitionColumnTypeNodes);
  ...
  public Rel toProtobuf() {
    ...
    if (!partitionColumnTypeNodes.isEmpty()) {
      PartitionColumns.Builder partitionColumnsBuilder = PartitionColumns.newBuilder();
      for (ColumnTypeNode columnTypeNode : partitionColumnTypeNodes) {
