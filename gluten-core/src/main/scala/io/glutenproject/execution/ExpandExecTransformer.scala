  ...
  def getRelNode(
    ...
    for (i <- 0 until aggSize) {
      val expr = ExpressionConverter
        .replaceWithExpressionTransformer(projections(0)(i), originalInputAttributes)
    ...
    projections.foreach {
      projection =>
        ...
        for (i <- aggSize until (projection.size - 1)) {
          if (
            !(projection(i).isInstanceOf[Literal] &&
              projection(i).asInstanceOf[Literal].value == null)
          ) {
            val expr = ExpressionConverter
              .replaceWithExpressionTransformer(projection(i), originalInputAttributes)
