...
trait HashJoinLikeExecTransformer
  ...
  val (buildKeyExprs, streamedKeyExprs) = {
    ...
    // But this improvement would cause extra projet before hash join in velox, disalbe
    // this improvement as below would help reduce the project.
    val lkeys = leftKeys // HashJoin.rewriteKeyExpr(leftKeys)
    val rkeys = rightKeys // HashJoin.rewriteKeyExpr(rightKeys)
    if (exchangeTable) {
      (lkeys, rkeys)
    } else {
      (rkeys, lkeys)
