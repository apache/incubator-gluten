...
trait HashJoinLikeExecTransformer
  ...
  override lazy val metrics = Map(
    ...
    "hashBuildCount" -> SQLMetrics.createMetric(sparkContext, "hash build cpu wall time count"),
    "hashBuildWallNanos" -> SQLMetrics.createNanoTimingMetric(sparkContext, "totaltime_hashbuild"),
    ...
    "hashProbeCount" -> SQLMetrics.createMetric(sparkContext, "hash probe cpu wall time count"),
    "hashProbeWallNanos" -> SQLMetrics.createNanoTimingMetric(sparkContext, "totaltime_hashprobe"),
    ...
    "postProjectionOutputBytes" -> SQLMetrics.createSizeMetric(
      sparkContext,
      "number of postProjection output bytes"),
  ...
  protected def createPreProjectionIfNeeded(
...
object HashJoinLikeExecTransformer {
  ...
  def makeIsNullExpression(
      childNode: ExpressionNode,
      functionMap: java.util.HashMap[String, java.lang.Long]): ExpressionNode = {
    val functionId = ExpressionBuilder.newScalarFunction(
      functionMap,
      ConverterUtils.makeFuncName(ConverterUtils.IS_NULL, Seq(BooleanType)))
...
abstract class ShuffledHashJoinExecTransformer(
