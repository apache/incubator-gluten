...
case class HashAggregateExecPullOutHelper(agg: BaseAggregateExec) extends AliasHelper {
  // The direct outputs of Aggregation.
  lazy val allAggregateResultAttributes: List[Attribute] =
    agg.groupingExpressions.map(ConverterUtils.getAttrFromExpr(_)).toList :::
      getAttrForAggregateExprs(agg.aggregateExpressions, agg.aggregateAttributes)
