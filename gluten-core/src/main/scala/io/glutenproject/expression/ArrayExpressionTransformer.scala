...
class SequenceTransformer(
  ...
  override def doTransform(args: java.lang.Object): ExpressionNode = {
    ...
    val getLiteralNode = (n: Long) =>
      original.start.dataType match {
    ...
    val lessThanFuncId = ExpressionBuilder.newScalarFunction(
      ...
      ConverterUtils.makeFuncName(
        ...
        FunctionConfig.OPT))
    val rangeFuncId = ExpressionBuilder.newScalarFunction(
      functionMap,
      ConverterUtils.makeFuncName(
        "range",
        Seq(original.start.dataType, original.stop.dataType, original.start.dataType),
        FunctionConfig.OPT))
    val addFuncId = ExpressionBuilder.newScalarFunction(
      functionMap,
      ConverterUtils.makeFuncName(
        ExpressionNames.ADD,
        Seq(original.stop.dataType, original.start.dataType),
        FunctionConfig.OPT))
    val substractFuncId = ExpressionBuilder.newScalarFunction(
      functionMap,
      ConverterUtils.makeFuncName(
        ExpressionNames.SUBTRACT,
        Seq(original.stop.dataType, original.start.dataType),
        FunctionConfig.OPT))
    val remainderFuncId = ExpressionBuilder.newScalarFunction(
      functionMap,
      ConverterUtils.makeFuncName(
        ExpressionNames.REMAINDER,
        Seq(original.stop.dataType, original.start.dataType),
        FunctionConfig.OPT))
    val equalFuncId = ExpressionBuilder.newScalarFunction(
      functionMap,
      ConverterUtils.makeFuncName(
    ...
    new IfThenNode(
      Lists.newArrayList(equalFuncNode),
      Lists.newArrayList(leftRangeFuncNode),
      rightRangeFuncNode)
  }
}
