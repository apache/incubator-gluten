    ...
    expr match {
      ...
      case a: AttributeReference =>
        ...
        if (bindReference == expr) {
          ...
          new AttributeReferenceTransformer(a.name, b.ordinal, a.dataType, b.nullable, a.metadata)(
            a.exprId,
            a.qualifier)
        }
      case l: Literal =>
      ...
      case cw: CaseWhen =>
        ...
        val colBranches = cw.branches.map {
          expr =>
            {
              (
        ...
        val colElseValue = cw.elseValue.map {
          expr =>
            {
      ...
      case c: Coalesce =>
        logInfo(s"${expr.getClass} $expr is supported")
        val exprs = c.children.map(expr => replaceWithExpressionTransformer(expr, attributeSeq))
      ...
      case u: UnaryExpression =>
        ...
        if (!u.isInstanceOf[CheckOverflow] || !u.child.isInstanceOf[Divide]) {
          ...
          val columnarDivide = BinaryArithmeticTransformer.createDivide(
            replaceWithExpressionTransformer(divide.left, attributeSeq),
            replaceWithExpressionTransformer(divide.right, attributeSeq),
            divide,
            u.dataType.asInstanceOf[DecimalType]
          )
          UnaryOperatorTransformer.create(columnarDivide, expr)
        }
      ...
      case g: Greatest =>
        logInfo(s"${expr.getClass} $expr is supported")
        val exprs = g.children.map(expr => replaceWithExpressionTransformer(expr, attributeSeq))
        new GreatestTransformer(exprs, expr)
      case l: Least =>
        logInfo(s"${expr.getClass} $expr is supported")
        val exprs = l.children.map(expr => replaceWithExpressionTransformer(expr, attributeSeq))
        new LeastTransformer(exprs, expr)
      case m: Murmur3Hash =>
        logInfo(s"${expr.getClass} $expr is supported")
        val exprs = m.children.map(expr => replaceWithExpressionTransformer(expr, attributeSeq))
