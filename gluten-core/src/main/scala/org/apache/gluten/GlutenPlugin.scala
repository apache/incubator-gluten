...
private object GlutenPlugin {
  ...
  private[gluten] class GlutenDriverPlugin extends DriverPlugin with Logging {
    ...
    override def init(sc: SparkContext, pluginContext: PluginContext): util.Map[String, String] = {
      ...
      // Exclude components.
      val excludedComponents = getExcludedComponents(conf)
      Component.excludeByNames(excludedComponents)

      // Register Gluten listeners.
      GlutenSQLAppStatusListener.register(sc)
      if (
        conf.getBoolean(GLUTEN_SOFT_AFFINITY_ENABLED, GLUTEN_SOFT_AFFINITY_ENABLED_DEFAULT_VALUE)
      ) {
  ...
  private[gluten] class GlutenExecutorPlugin extends ExecutorPlugin {
    ...
    override def init(ctx: PluginContext, extraConf: util.Map[String, String]): Unit = {
      // Exclude components.
      val excludedComponents = getExcludedComponents(ctx.conf())
      Component.excludeByNames(excludedComponents)
