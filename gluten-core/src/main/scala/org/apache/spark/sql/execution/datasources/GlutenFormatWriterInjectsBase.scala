...
import io.glutenproject.extension.columnar.{AddTransformHintRule, ColumnarPullOutPreProject}
...
trait GlutenFormatWriterInjectsBase extends GlutenFormatWriterInjects {
  ...
  override def executeWriterWrappedSparkPlan(plan: SparkPlan): RDD[InternalRow] = {
    ...
    val pulledOutPlan = ColumnarPullOutPreProject(plan.session)(plan)
    val transformed =
