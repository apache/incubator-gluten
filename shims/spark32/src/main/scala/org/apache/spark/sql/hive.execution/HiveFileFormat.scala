...
/**
 * This class is copied from Spark 3.2.2 and modified for Gluten. Gluten should make sure this class
...
class HiveFileFormat(fileSinkConf: FileSinkDesc)
  ...
  override def prepareWrite(
    ...
    if (
      sparkSession.sessionState.conf.getConf(NATIVE_PARQUET_WRITER_ENABLED) &&
      fileSinkConf.tableInfo.getOutputFileFormatClassName
        .equals("org.apache.hadoop.hive.ql.io.parquet.MapredParquetOutputFormat")
    ) {
      GlutenOutputWriterFactoryCreator.getInstance().createFactory(shortName())
    } else {
