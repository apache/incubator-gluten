...
abstract class QueryPlan[PlanType <: QueryPlan[PlanType]]
  ...
  override def simpleStringWithNodeId(): String = {
    val operatorId = getTagValue(QueryPlanWrapper.OP_ID_TAG).map(id => s"$id").getOrElse("unknown")
  ...
  protected def formattedNodeName: String = {
    val opId = getTagValue(QueryPlanWrapper.OP_ID_TAG).map(id => s"$id").getOrElse("unknown")
    val codegenId =
  ...
  def subqueries: Seq[PlanType] = {
    expressions.flatMap(_.collect { case e: PlanExpression[_] => e.plan.asInstanceOf[PlanType] })
  }
