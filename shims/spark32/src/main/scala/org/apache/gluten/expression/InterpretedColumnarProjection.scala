...
import org.apache.spark.sql.catalyst.expressions.{Attribute, Expression, Nondeterministic, SubExprEvaluationRuntime}
...
class InterpretedColumnarProjection(expressions: Seq[Expression]) extends ArrowProjection {
  ...
  def currentValue: ColumnarRow = mutableRow
