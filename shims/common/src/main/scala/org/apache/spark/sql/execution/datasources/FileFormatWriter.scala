        ...
        if (concurrentWritersEnabled) {
          ...
          if (!inputIsColumnar)
            ...
            if (bucketIdExpression.isDefined) {
              // We need to add the bucket id expression to the output of the sort plan, so that we can use
              // backend to calculate the bucket id for each row.
              wrapped = ProjectExec(
                wrapped.output :+ Alias(bucketIdExpression.get, "_bucket_value_")(),
                wrapped)
            }
