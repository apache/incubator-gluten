...
class GlutenConfig(conf: SQLConf) extends Logging {
  ...
  def rasRoughSizeBytesThreshold: Long = conf.getConf(RAS_ROUGH_SIZEBYTES_THRESHOLD)

  def rasRoughR2cCost: Long = conf.getConf(RAS_ROUGH_R2C_COST)

  def rasRoughVanillaCost: Long = conf.getConf(RAS_ROUGH_VANILLA_COST)
...
object GlutenConfig {
  ...
  val RAS_ROUGH_SIZEBYTES_THRESHOLD =
    buildConf("spark.gluten.ras.rough.sizeBytesThreshold")
      .doc(
        "Experimental: Threshold of the byte size consumed by sparkPlan, coefficient used " +
          "to calculate cost in RAS rough model")
      .longConf
      .createWithDefault(1073741824L)

  val RAS_ROUGH_R2C_COST =
    buildConf("spark.gluten.ras.rough.r2c.cost")
      .doc("Experimental: Cost of RowToVeloxColumnarExec in RAS rough model")
      .longConf
      .createWithDefault(100L)

  val RAS_ROUGH_VANILLA_COST =
    buildConf("spark.gluten.ras.rough.vanilla.cost")
