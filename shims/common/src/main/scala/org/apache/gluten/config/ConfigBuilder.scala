...
private object ConfigHelpers {
  ...
  def timeFromString(str: String, unit: TimeUnit): Long = {
    ...
    try {
      val matcher = TIME_STRING_PATTERN.matcher(lower)
      if (!matcher.matches()) {
        throw new NumberFormatException("Failed to parse time string: " + str)
      }

      val suffix = matcher.group(2)
      val timeUnit = if (suffix == null) {
        unit
      } else {
        matcher.group(2) match {
          ...
          case suffix => throw new NumberFormatException(s"Invalid suffix: $suffix. ")
        }
      }
      unit.convert(matcher.group(1).toLong, timeUnit)
    } catch {
