...
/**
 * Copied from vanilla spark's HashJoin and only Specifically disabled the rewriting feature in
 * HashJoin#canRewriteAsLongType
 */
    ...
    require(
      leftKeys.length == rightKeys.length &&
        leftKeys
          ...
          .forall(
            types => DataType.equalsStructurally(types._1, types._2, ignoreNullability = true)),
...
object HashJoin extends CastSupport with SQLConfHelper {
  ...
  private def canRewriteAsLongType(keys: Seq[Expression]): Boolean = {
    val confMap = conf.getAllConfs
    val enableRewriteAsLong: Boolean =
      confMap.getOrElse("spark.gluten.sql.hashjoin.key.rewrite", "false").toBoolean
    // TODO: support BooleanType, DateType and TimestampType
    keys.forall(_.dataType.isInstanceOf[IntegralType]) &&
    keys.map(_.dataType.defaultSize).sum <= 8 && enableRewriteAsLong
  }
