...
import io.glutenproject.GlutenConfig.NATIVE_PARQUET_WRITER_ENABLED
...
class ParquetFileFormat extends FileFormat with DataSourceRegister with Logging with Serializable {
  ...
  override def prepareWrite(
    ...
    if (sparkSession.sessionState.conf.getConf(NATIVE_PARQUET_WRITER_ENABLED)) {
      ...
      options.get(GlutenConfig.PARQUET_BLOCK_SIZE).foreach {
        blockSize => sparkOptions.put(GlutenConfig.PARQUET_BLOCK_SIZE, blockSize)
      }
      GlutenOutputWriterFactoryCreator.getInstance().createFactory(shortName(), sparkOptions.toMap)
    } else {
