...
// copied from Spark-352, modifed to L#195 to pass azure config
object SQLExecution {
  ...
  /**
   * Wrap an action with a known executionId. When running a different action in a different thread
   * from the original one, this method can be used to connect the Spark jobs in this action with
   * the known executionId, e.g., `BroadcastExchangeExec.relationFuture`.
   */
  ...
  def withSQLConfPropagated[T](sparkSession: SparkSession)(body: => T): T = {
    ...
    val originalLocalProps = allConfigs.collect {
      case (key, value) if (key.startsWith("spark") || key.startsWith("fs.azure.account")) =>
