...
static jclass Java_VeloxBroadcastBuildSideCache = nullptr;
...
jlong callJavaGet(const std::string& id) {
  ...
  auto result = env->CallStaticLongMethod(Java_VeloxBroadcastBuildSideCache, Java_get, s);
  return result;
}
...
void initVeloxJniHashTable(JNIEnv* env) {
  ...
  Java_VeloxBroadcastBuildSideCache = createGlobalClassReferenceOrError(env, classSig);
  Java_get = getStaticMethodId(env, Java_VeloxBroadcastBuildSideCache, "get", "(Ljava/lang/String;)J");
}

void finalizeVeloxJniHashTable(JNIEnv* env) {
  env->DeleteGlobalRef(Java_VeloxBroadcastBuildSideCache);
}
