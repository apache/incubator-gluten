...
int32_t VeloxExecutionCtx::getColumnarBatchPerRowSize(std::shared_ptr<ColumnarBatch> cb) {
  auto veloxBatch = std::dynamic_pointer_cast<VeloxColumnarBatch>(cb);
  VELOX_DCHECK(veloxBatch != nullptr, "getColumnarBatchPerRowSize batch should be VeloxColumnarBatch");
  auto rowVector = veloxBatch->getFlattenedRowVector();
  auto fast = std::make_unique<facebook::velox::row::UnsafeRowFast>(rowVector);
  return fast->rowSize(0);
}

std::shared_ptr<ColumnarBatch> VeloxExecutionCtx::getNonPartitionedColumnarBatch(
    std::shared_ptr<ColumnarBatch> cb,
    std::vector<size_t> partitionColIndiceVec,
    MemoryManager* memoryManager,
    char*& rowBytes) {
  auto veloxBatch = std::dynamic_pointer_cast<VeloxColumnarBatch>(cb);
  VELOX_DCHECK(veloxBatch != nullptr, "getNonPartitionedColumnarBatch batch should be VeloxColumnarBatch");
  auto rowVector = veloxBatch->getFlattenedRowVector();
  auto childrens = rowVector->children();
  auto fast = std::make_unique<facebook::velox::row::UnsafeRowFast>(rowVector);
  fast->serialize(0, rowBytes);

  for (int i = 0; i < partitionColIndiceVec.size(); i++) {
    auto index = partitionColIndiceVec[i];
  ...
  std::vector<std::string> newNames(names.begin(), names.end());
  auto typeChildren = rowType->children();
  for (int i = 0; i < partitionColIndiceVec.size(); i++) {
    typeChildren.erase(typeChildren.begin() + partitionColIndiceVec[i]);
    newNames.erase(newNames.begin() + partitionColIndiceVec[i]);
  }
  auto newType = facebook::velox::TypeFactory<TypeKind::ROW>::create(std::move(newNames), std::move(typeChildren));
  auto veloxPool = getLeafVeloxPool(memoryManager);

  auto tmpRowVector = std::make_shared<facebook::velox::RowVector>(
      rowVector->pool(), newType, nullptr, rowVector->size(), std::move(childrens));

  // The pool in children here is derived from the previous operator. We need a copy operation to ensure that the pool
  // for the final columnar batch originates from veloxPool. Otherwise, there may be a possibility of premature pool
  // release during destruction
  velox::RowVectorPtr finalRowVector = std::dynamic_pointer_cast<facebook::velox::RowVector>(
      facebook::velox::BaseVector::create(tmpRowVector->type(), tmpRowVector->size(), veloxPool.get()));
  finalRowVector->copy(tmpRowVector.get(), 0, 0, tmpRowVector->size());
  return std::make_shared<VeloxColumnarBatch>(finalRowVector);
}
