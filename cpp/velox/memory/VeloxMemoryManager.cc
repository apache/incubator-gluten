...
const MemoryUsageStats VeloxMemoryManager::collectMemoryUsageStats() const {
  ...
  stats.mutable_children()->emplace(
      "__sys_spilling__", collectVeloxMemoryUsageStats(veloxMemoryManager_->spillPool()));
  stats.mutable_children()->emplace(
      "__sys_caching__", collectVeloxMemoryUsageStats(veloxMemoryManager_->cachePool()));
  stats.mutable_children()->emplace(
      "__sys_tracing__", collectVeloxMemoryUsageStats(veloxMemoryManager_->tracePool()));
  stats.mutable_children()->emplace(
