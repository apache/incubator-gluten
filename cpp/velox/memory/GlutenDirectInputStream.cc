...
GlutenDirectInputStream::GlutenDirectInputStream(
    GlutenDirectBufferedInput* bufferedInput,
...
bool GlutenDirectInputStream::Next(const void** buffer, int32_t* size) {
...
void GlutenDirectInputStream::BackUp(int32_t count) {
...
bool GlutenDirectInputStream::SkipInt64(int64_t count) {
  if (count < 0) {
    return false;
  }
  const uint64_t unsignedCount = static_cast<uint64_t>(count);
...
int64_t GlutenDirectInputStream::ByteCount() const {
  return static_cast<int64_t>(offsetInRegion_);
}

void GlutenDirectInputStream::seekToPosition(PositionProvider& seekPosition) {
  offsetInRegion_ = seekPosition.next();
  VELOX_CHECK_LE(offsetInRegion_, region_.length);
}

std::string GlutenDirectInputStream::getName() const {
  return fmt::format("GlutenDirectInputStream {} of {}", offsetInRegion_, region_.length);
}

size_t GlutenDirectInputStream::positionSize() const {
...
makeRanges(size_t size, facebook::velox::memory::Allocation& data, std::string& tinyData) {
  ...
  if (data.numPages() > 0) {
    ...
    for (int i = 0; i < data.numRuns(); ++i) {
      ...
      uint64_t bytes = facebook::velox::memory::AllocationTraits::pageBytes(run.numPages());
...
void GlutenDirectInputStream::loadSync() {
  if (region_.length < GlutenDirectBufferedInput::kTinySize && data_.numPages() == 0) {
    tinyData_.resize(region_.length);
  } else {
    const auto numPages = facebook::velox::memory::AllocationTraits::numPages(loadedRegion_.length);
    if (numPages > data_.numPages()) {
      bufferedInput_->pool()->allocateNonContiguous(numPages, data_);
    }
  }

  facebook::velox::process::TraceContext trace("DirectInputStream::loadSync");

  ioStats_->incRawBytesRead(loadedRegion_.length);
  auto ranges = makeRanges(loadedRegion_.length, data_, tinyData_);
  uint64_t usecs = 0;
  {
    facebook::velox::MicrosecondTimer timer(&usecs);
...
void GlutenDirectInputStream::loadPosition() {
  ...
  if (!loaded_) {
    ...
    if (load != nullptr) {
      ...
      uint64_t loadUs = 0;
      {
        facebook::velox::MicrosecondTimer timer(&loadUs);
  ...
  if (loadedRegion_.length == 0 || region_.offset + offsetInRegion_ < loadedRegion_.offset ||
      region_.offset + offsetInRegion_ >= loadedRegion_.offset + loadedRegion_.length) {
    loadedRegion_.offset = region_.offset + offsetInRegion_;
    loadedRegion_.length =
        (offsetInRegion_ + loadQuantum_ <= region_.length) ? loadQuantum_ : (region_.length - offsetInRegion_);
  ...
  const auto offsetInData = offsetInRegion_ - (loadedRegion_.offset - region_.offset);
  if (data_.numPages() == 0) {
    ...
    run_ = run.data();
    runSize_ = facebook::velox::memory::AllocationTraits::pageBytes(run.numPages());
