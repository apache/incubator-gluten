...
/// GlutenDirectBufferedInput. Similar to CacheInputStream but does not use cache.
class GlutenDirectInputStream : public facebook::velox::dwio::common::SeekableInputStream {
 public:
  GlutenDirectInputStream(
      GlutenDirectBufferedInput* bufferedInput,
      facebook::velox::io::IoStatistics* ioStats,
      const facebook::velox::common::Region& region,
      ...
      facebook::velox::cache::TrackingId trackingId,
      uint64_t groupId,
      int32_t loadQuantum);
  ...
  void seekToPosition(facebook::velox::dwio::common::PositionProvider& position) override;
  std::string getName() const override;
  size_t positionSize() const override;

  /// Testing function to access loaded state.
  void testingData(
      facebook::velox::common::Region& loadedRegion,
      facebook::velox::memory::Allocation*& data,
      std::string*& tinyData) {
 ...
 private:
  ...
  GlutenDirectBufferedInput* const bufferedInput_;
  facebook::velox::io::IoStatistics* const ioStats_;
  const std::shared_ptr<facebook::velox::dwio::common::ReadFileInputStream> input_;
  // The region of 'input' 'this' ranges over.
  const facebook::velox::common::Region region_;
  const uint64_t fileNum_;
  std::shared_ptr<facebook::velox::cache::ScanTracker> tracker_;
  const facebook::velox::cache::TrackingId trackingId_;
