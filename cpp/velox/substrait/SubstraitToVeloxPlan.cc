...
void SubstraitToVeloxPlanConverter::setColInfoMap(
    const std::string& filterName,
    uint32_t colIdx,
...
void SubstraitToVeloxPlanConverter::setFilterMap(
  ...
  switch (inputType->kind()) {
    case TypeKind::INTEGER:
      ...
      setColInfoMap(functionName, colIdxVal, val, reverse, column2FilterInfo);
      break;
...
void SubstraitToVeloxPlanConverter::constructSubfieldFilters(
  ...
  if (filterInfo.notValue_) {
    if constexpr (KIND == facebook::velox::TypeKind::HUGEINT) {
      VELOX_NYI("HUGEINT not support notValue filter");
    } else {
  ...
  if constexpr (KIND != facebook::velox::TypeKind::HUGEINT) {
    ...
    } else if (KIND == facebook::velox::TypeKind::HUGEINT) {
      if (inputType->isLongDecimal()) {
    ...
    if constexpr (KIND == facebook::velox::TypeKind::BIGINT) {
      ...
      if (inputType->isLongDecimal()) {
        ...
        upperBound = getLowest<NativeType>();
      }
    } else {
      upperBound = getMax<NativeType>();
    }
