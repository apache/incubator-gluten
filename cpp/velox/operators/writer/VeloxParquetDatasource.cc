...
void VeloxParquetDatasource::init(const std::unordered_map<std::string, std::string>& sparkConfs) {
  ...
  //         "The write path is hdfs path but the HDFS haven't been enabled when writing parquet data in velox
  //         backend!");
  // #endif
  ...
  //   // Setting the ratio to 2 here refers to the grow strategy in the reserve() method of MemoryPool on the arrow
  //   side. std::unordered_map<std::string, std::string> configData({{velox::core::QueryConfig::kDataBufferGrowRatio,
  //   "2"}}); auto queryCtx = std::make_shared<velox::core::QueryCtx>(nullptr, configData);
