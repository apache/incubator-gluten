...
class VeloxShuffleWriterTestBase : public facebook::velox::test::VectorTestBase {
 public:
  virtual arrow::Status initShuffleWriterOptions() {
    ...
    shuffleWriterOptions_->memoryPool = defaultArrowMemoryPool().get();
...
class VeloxShuffleWriterTest : public ::testing::TestWithParam<ShuffleTestParams>, public VeloxShuffleWriterTestBase {
 public:
  arrow::Status initShuffleWriterOptions() override {
    ...
    if (params.partitionWriterType == PartitionWriterType::kCeleborn) {
      shuffleWriterOptions_->partitionWriterType = kCeleborn;
    }
    shuffleWriterOptions_->compressionType = params.compressionType;
 ...
 protected:
  ...
  void getRowVectors(
    ...
    options.compressionType = writerOptions->compressionType;
    ...
    // Set batchSize to a large value to make sure batches are merged by reader.
    auto deserializerFactory = std::make_unique<gluten::VeloxColumnarBatchDeserializerFactory>(
        schema, std::move(codec), rowType, kDefaultShuffleWriterBufferSize << 1, defaultArrowMemoryPool().get(), pool_);
...
class SinglePartitioningShuffleWriter : public VeloxShuffleWriterTest {
 protected:
  void testShuffleWrite(VeloxShuffleWriter& shuffleWriter, std::vector<facebook::velox::RowVectorPtr> vectors) {
    ...
    getRowVectors(shuffleWriter.options(), schema, deserializedVectors, in);
  ...
  std::shared_ptr<VeloxShuffleWriter> createShuffleWriter() override {
    ...
    auto partitionWriter = createPartitionWriter(
        shuffleWriterOptions_->partitionWriterType, kNumPartitions, dataFile_, localDirs_, shuffleWriterOptions_.get());
...
class VeloxShuffleWriterMemoryTest : public VeloxShuffleWriterTestBase, public testing::Test {
 protected:
  ...
  std::shared_ptr<VeloxShuffleWriter> createShuffleWriter(uint32_t numPartitions) {
    auto partitionWriter = createPartitionWriter(
        shuffleWriterOptions_->partitionWriterType, numPartitions, dataFile_, localDirs_, shuffleWriterOptions_.get());
    GLUTEN_ASSIGN_OR_THROW(
        auto shuffleWriter,
