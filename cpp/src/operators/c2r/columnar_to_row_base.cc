...
int64_t ColumnarToRowConverterBase::RoundNumberOfBytesToNearestWord(int64_t numBytes) {
...
int64_t ColumnarToRowConverterBase::GetFieldOffset(int64_t nullBitsetWidthInBytes,
                                                   int32_t index) {
...
void ColumnarToRowConverterBase::SetNullAt(uint8_t* buffer_address, int64_t row_offset,
                                           int64_t field_offset, int32_t col_index) {
...
int32_t ColumnarToRowConverterBase::FirstNonzeroLongNum(std::vector<int32_t> mag,
                                                        int32_t length) {
  int32_t fn = 0;
  int32_t i;
...
int32_t ColumnarToRowConverterBase::GetInt(int32_t n, int32_t sig,
                                           std::vector<int32_t> mag, int32_t length) {
  if (n < 0) return 0;
...
int32_t ColumnarToRowConverterBase::GetBitLength(int32_t sig, std::vector<int32_t> mag,
                                                 int32_t len) {
  int32_t n = -1;
  if (len == 0) {
    n = 0;
...
std::vector<uint32_t> ColumnarToRowConverterBase::ConvertMagArray(int64_t new_high,
                                                                  uint64_t new_low,
                                                                  int32_t* size) {
...
std::array<uint8_t, 16> ColumnarToRowConverterBase::ToByteArray(arrow::Decimal128 value,
                                                                int32_t* length) {
