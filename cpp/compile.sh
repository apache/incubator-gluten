...
do
    case $arg in
        ...
        shift # Remove argument name from processing
        ;;
        --build_gazelle_backend=*)
        BUILD_GAZELLE_BACKEND=("${arg#*=}")
        shift # Remove argument name from processing
        ;;
        --build_velox_backend=*)
...
echo "BUILD_GAZELLE_BACKEND=${BUILD_GAZELLE_BACKEND}"
...
cmake .. \
  ...
  -DBUILD_GAZELLE_BACKEND=${BUILD_GAZELLE_BACKEND} \
