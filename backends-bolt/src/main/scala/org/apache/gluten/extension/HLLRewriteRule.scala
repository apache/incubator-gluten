...
case class HLLRewriteRule(spark: SparkSession) extends Rule[LogicalPlan] {
  ...
  private def isSupportedDataType(dataType: DataType): Boolean = {
    // HLL in Bolt only supports below data types. We should not offload HLL to bolt, if
    // child's data type is not supported. This prevents the case only partail agg is fallbacked.
    // As Spark and Bolt have different HLL binary formats, HLL binary generated by Spark can't
    // be parsed by Bolt, it would cause the error: 'Unexpected type of HLL'.
    dataType match {
