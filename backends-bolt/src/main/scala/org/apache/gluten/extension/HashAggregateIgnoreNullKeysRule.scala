 ...
 * ignoreNullKeys to true when convert to bolt's AggregateNode.
 */
case class HashAggregateIgnoreNullKeysRule(session: SparkSession) extends Rule[SparkPlan] {
  override def apply(plan: SparkPlan): SparkPlan = {
    if (!BoltConfig.get.enablePropagateIgnoreNullKeys) {
      return plan
    }
    plan.transformUp {
  ...
  private def canPropagate(plan: SparkPlan): Boolean = plan match {
    ...
    case _: BoltResizeBatchesExec => true
    case _: ShuffleExchangeLike => true
    case _: BoltColumnarToRowExec => true
