...
{
    readSchema<ParquetSchemaReader>(TEST_DATA(/alltypes/alltypes_notnull.parquet));
    readSchema<ParquetSchemaReader>(TEST_DATA(/alltypes/alltypes_null.parquet));
    readSchema<OptimizedParquetSchemaReader>(TEST_DATA(/alltypes/alltypes_null.parquet));
    readSchema<OptimizedParquetSchemaReader>(TEST_DATA(/alltypes/alltypes_null.parquet));
}

TEST(ParquetRead, ReadDataNotNull)
{
    const String path = TEST_DATA(/alltypes/alltypes_notnull.parquet);
...
{
    const String path = TEST_DATA(/alltypes/alltypes_null.parquet);
