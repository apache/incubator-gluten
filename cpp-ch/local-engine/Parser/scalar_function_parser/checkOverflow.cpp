...
public:
    ...
    String getCHFunctionName(const substrait::Expression_ScalarFunction & func) const override
    {
        ...
        if (args.size() < 2)
            throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "check_overflow function requires at least two arguments");
        String ch_function_name = "checkDecimalOverflowSpark";
        auto null_on_overflow = args[1].value().literal().boolean();
        if (null_on_overflow)
...
static FunctionParserRegister<SparkFunctionCheckOverflow> register_check_overflow;
}
