...
TEST(ParquetRead, ReadDataNotNull)
{
    ...
    readData<OptimizedParquetSchemaReader, local_engine::VectorizedParquetBlockInputFormat>(
        "alltypes/alltypes_notnull.parquet", primitive_fields);
...
using RowType = DB::ColumnsWithTypeAndName;
...
TEST(ParquetRead, VectorizedColumnReader)
{
    ...
    recordReader.initialize(blockHeader, arrow_file);
...
TEST(ParquetRead, ReadDict)
{
    GTEST_SKIP();
    Field field1{Int64(1)};
    const std::map<String, Field> fields{{"l_suppkey", field1}};

    readData<ParquetSchemaReader, local_engine::ArrowParquetBlockInputFormat>(
        "/home/chang/test/tpch/parquet/s10/lineitem/part-00000-f83d0a59-2bff-41bc-acde-911002bf1b33-c000.snappy.parquet", fields);
}
#endif
