...
{
    if (const auto date32 = typeid_cast<const DB::SerializationDate32 *>(nested_ptr.get()))
    {
    ...
    else if (auto uint16 = typeid_cast<const SerializationNumber<DB::UInt16> *>(nested_ptr.get()))
    {
        deserializeNumberTextCSV<DB::UInt16>(column, istr, settings);
    }
    else if (auto uint32 = typeid_cast<const SerializationNumber<DB::UInt32> *>(nested_ptr.get()))
    {
        deserializeNumberTextCSV<DB::UInt32>(column, istr, settings);
    }
    else if (auto uint64 = typeid_cast<const SerializationNumber<DB::UInt64> *>(nested_ptr.get()))
    {
        deserializeNumberTextCSV<DB::UInt64>(column, istr, settings);
    }
    else if (auto int8 = typeid_cast<const SerializationNumber<DB::Int8> *>(nested_ptr.get()))
    {
        deserializeNumberTextCSV<DB::Int8>(column, istr, settings);
    }
    else if (auto int16 = typeid_cast<const SerializationNumber<DB::Int16> *>(nested_ptr.get()))
    {
        deserializeNumberTextCSV<DB::Int16>(column, istr, settings);
    }
    else if (auto int32 = typeid_cast<const SerializationNumber<DB::Int32> *>(nested_ptr.get()))
    {
        deserializeNumberTextCSV<DB::Int32>(column, istr, settings);
    }
    else if (auto int64 = typeid_cast<const SerializationNumber<DB::Int64> *>(nested_ptr.get()))
    {
        deserializeNumberTextCSV<DB::Int64>(column, istr, settings);
    }
    else if (auto float32 = typeid_cast<const SerializationNumber<DB::Float32> *>(nested_ptr.get()))
    {
        deserializeNumberTextCSV<DB::Float32>(column, istr, settings);
    }
    else if (auto float64 = typeid_cast<const SerializationNumber<DB::Float64> *>(nested_ptr.get()))
    {
    ...
    const DateLUTImpl & utc_time_zone) const
{
    ...
    local_engine::readDateTime64Text(x, istr, settings, time_zone, utc_time_zone);
