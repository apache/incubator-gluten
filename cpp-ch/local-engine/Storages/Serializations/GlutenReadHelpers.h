...
template <typename T>
inline void readGlutenFloatText(T & x, DB::ReadBuffer & buf, const DB::FormatSettings & settings)
{
    {
        char * pos = buf.position();
        if (local_engine::readGlutenFloatTextFastImpl(x, buf, settings))
            return;
        else
            buf.position() = pos;
...
template <typename T>
inline void readGlutenIntegerText(T & x, DB::ReadBuffer & buf, const DB::FormatSettings & settings)
{
    if constexpr (std::is_same_v<decltype(x), bool &>)
        readBoolText(x, buf);
    else
        readGlutenIntTextImpl(x, buf, settings);
}

inline void readGlutenText(is_floating_point auto & x, DB::ReadBuffer & buf, const DB::FormatSettings & settings)
{
    readGlutenFloatText(x, buf, settings);
}

inline void readGlutenText(is_integer auto & x, DB::ReadBuffer & buf, const DB::FormatSettings & settings)
{
    readGlutenIntegerText(x, buf, settings);
}

inline void readGlutenText(LocalDate & x, DB::ReadBuffer & buf, const DB::FormatSettings & settings)
{
