...
inline uint64_t ALWAYS_INLINE UnpackValue(const uint8_t* __restrict__ in_buf) {
    ...
    constexpr int LAST_WORD_IDX = round_up_numi32(LAST_BIT_IDX);
    ...
    // compiler version.
    constexpr bool READ_32_BITS = WORDS_TO_READ == 1 && (!CAN_SAFELY_READ_64_BITS || isPowerOf2(BIT_WIDTH));
...
const uint8_t* BitPacking::UnpackUpTo31Values(const uint8_t* __restrict__ in, int64_t in_bytes, int num_values,
    ...
    if (round_up(BYTES_TO_READ, sizeof(uint32_t)) > in_bytes) {
