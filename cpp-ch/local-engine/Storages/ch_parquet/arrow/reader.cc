...
namespace arrow
{
    ...
    namespace
    {
        ...
        ::arrow::Result<std::shared_ptr<ArrayData>> ChunksToSingle(const ChunkedArray & chunked)
        {
            ...
            {
                case 0: {
                    ARROW_ASSIGN_OR_RAISE(std::shared_ptr<Array> array, ::arrow::MakeArrayOfNull(chunked.type(), 0));
                    return array->data();
                }
                case 1:
                ...
                default:
                    ...
                    return Status::NotImplemented("Nested data conversions not implemented for chunked array outputs");
            }
        }
    ...
    public:
        ...
        ::arrow::Status NextBatch(int64_t batch_size, std::shared_ptr<::arrow::ChunkedArray> * out) final
        {
        ...
        virtual ::arrow::Status BuildArray(int64_t length_upper_bound, std::shared_ptr<::arrow::ChunkedArray> * out) = 0;
    ...
    namespace
    {
        ...
        // Forward declaration
        Status
        GetReader(const SchemaField & field, const std::shared_ptr<ReaderContext> & context, std::unique_ptr<ColumnReaderImpl> * out);
        ...
        {
        public:
            FileReaderImpl(MemoryPool * pool, std::unique_ptr<ParquetFileReader> reader, ArrowReaderProperties properties)
                : pool_(pool), reader_(std::move(reader)), reader_properties_(std::move(properties))
            {
            }
            ...
            Status Init()
            {
                return SchemaManifest::Make(
                    reader_->metadata()->schema(), reader_->metadata()->key_value_metadata(), reader_properties_, &manifest_);
            }

            FileColumnIteratorFactory SomeRowGroupsFactory(std::vector<int> row_groups)
            {
                return [row_groups](int i, ParquetFileReader * reader) { return new FileColumnIterator(i, reader, row_groups); };
            }

            FileColumnIteratorFactory AllRowGroupsFactory() { return SomeRowGroupsFactory(Iota(reader_->metadata()->num_row_groups())); }
            ...
            Status BoundsCheckRowGroup(int row_group)
            {
                ...
                {
                    return Status::Invalid(
                        "Some index in row_group_indices is ",
                        row_group,
            ...
            Status BoundsCheck(const std::vector<int> & row_groups, const std::vector<int> & column_indices)
            {
            ...
            Status ReadTable(const std::vector<int> & indices, std::shared_ptr<Table> * out) override
            {
                ...
                std::shared_ptr<::arrow::Schema> * out_schema)
            {
                ...
                ARROW_ASSIGN_OR_RAISE(std::vector<int> field_indices, manifest_.GetFieldIndices(column_indices));
            ...
            Status GetColumn(int i, FileColumnIteratorFactory iterator_factory, std::unique_ptr<ColumnReader> * out);

            Status GetColumn(int i, std::unique_ptr<ColumnReader> * out) override { return GetColumn(i, AllRowGroupsFactory(), out); }

            Status GetSchema(std::shared_ptr<::arrow::Schema> * out) override
            {
                return FromParquetSchema(reader_->metadata()->schema(), reader_properties_, reader_->metadata()->key_value_metadata(), out);
            }
            ...
            Status ReadColumn(int i, const std::vector<int> & row_groups, ColumnReader * reader, std::shared_ptr<ChunkedArray> * out)
            {
                ...
                {
                    // Can throw exception
                    records_to_read += reader_->metadata()->RowGroup(row_group)->ColumnChunk(i)->num_values();
                }
                return reader->NextBatch(records_to_read, out);
                END_PARQUET_CATCH_EXCEPTIONS
            }

            Status ReadColumn(int i, const std::vector<int> & row_groups, std::shared_ptr<ChunkedArray> * out)
            {
            ...
            Status ReadTable(std::shared_ptr<Table> * table) override { return ReadTable(Iota(reader_->metadata()->num_columns()), table); }

            Status
            ReadRowGroups(const std::vector<int> & row_groups, const std::vector<int> & indices, std::shared_ptr<Table> * table) override;
            ...
            Status ReadRowGroups(const std::vector<int> & row_groups, std::shared_ptr<Table> * table) override
            {
                return ReadRowGroups(row_groups, Iota(reader_->metadata()->num_columns()), table);
            }

            Status ReadRowGroup(int row_group_index, const std::vector<int> & column_indices, std::shared_ptr<Table> * out) override
            {
            ...
            Status GetRecordBatchReader(const std::vector<int> & row_group_indices, std::unique_ptr<RecordBatchReader> * out) override
            {
                return GetRecordBatchReader(row_group_indices, Iota(reader_->metadata()->num_columns()), out);
            }

            ::arrow::Result<::arrow::AsyncGenerator<std::shared_ptr<::arrow::RecordBatch>>> GetRecordBatchGenerator(
            ...
            void set_use_threads(bool use_threads) override { reader_properties_.set_use_threads(use_threads); }

            void set_batch_size(int64_t batch_size) override { reader_properties_.set_batch_size(batch_size); }
            ...
            Status ScanContents(std::vector<int> columns, const int32_t column_batch_size, int64_t * num_rows) override
            {
                BEGIN_PARQUET_CATCH_EXCEPTIONS
        ...
        {
        public:
            RowGroupRecordBatchReader(::arrow::RecordBatchIterator batches, std::shared_ptr<::arrow::Schema> schema)
            ...
            Status ReadNext(std::shared_ptr<::arrow::RecordBatch> * out) override { return batches_.Next().Value(out); }
        ...
        {
        public:
            RowGroupReaderImpl(FileReaderImpl * impl, int row_group_index) : impl_(impl), row_group_index_(row_group_index) { }

            std::shared_ptr<ColumnChunkReader> Column(int column_index) override
            {
                return std::shared_ptr<ColumnChunkReader>(new ColumnChunkReaderImpl(impl_, row_group_index_, column_index));
            }

            Status ReadTable(const std::vector<int> & column_indices, std::shared_ptr<::arrow::Table> * out) override
            {
                return impl_->ReadRowGroup(row_group_index_, column_indices, out);
            }

            Status ReadTable(std::shared_ptr<::arrow::Table> * out) override { return impl_->ReadRowGroup(row_group_index_, out); }
        ...
        public:
            LeafReader(
                ...
                : ctx_(std::move(ctx)), field_(std::move(field)), input_(std::move(input)), descr_(input_->descr())
            {
                record_reader_ = RecordReader::Make(descr_, leaf_info, ctx_->pool, field_->type()->id() == ::arrow::Type::DICTIONARY);
                NextRowGroup();
            }
            ...
            Status LoadBatch(int64_t records_to_read) final
            {
                ...
                RETURN_NOT_OK(TransferColumnData(record_reader_.get(), field_->type(), descr_, ctx_->pool, &out_));
                return Status::OK();
                END_PARQUET_CATCH_EXCEPTIONS
            }

            ::arrow::Status BuildArray(int64_t length_upper_bound, std::shared_ptr<::arrow::ChunkedArray> * out) final
            {
                *out = out_;
                return Status::OK();
            }
        ...
        {
        public:
            ExtensionReader(std::shared_ptr<Field> field, std::unique_ptr<ColumnReaderImpl> storage_reader)
                : field_(std::move(field)), storage_reader_(std::move(storage_reader))
            {
            }

            Status GetDefLevels(const int16_t ** data, int64_t * length) override { return storage_reader_->GetDefLevels(data, length); }

            Status GetRepLevels(const int16_t ** data, int64_t * length) override { return storage_reader_->GetRepLevels(data, length); }

            Status LoadBatch(int64_t number_of_records) final { return storage_reader_->LoadBatch(number_of_records); }

            Status BuildArray(int64_t length_upper_bound, std::shared_ptr<ChunkedArray> * out) override
            {
            ...
            bool IsOrHasRepeatedChild() const final { return storage_reader_->IsOrHasRepeatedChild(); }
        ...
        public:
            ListReader(
                ...
                : ctx_(std::move(ctx)), field_(std::move(field)), level_info_(level_info), item_reader_(std::move(child_reader))
            {
            }

            Status GetDefLevels(const int16_t ** data, int64_t * length) override { return item_reader_->GetDefLevels(data, length); }

            Status GetRepLevels(const int16_t ** data, int64_t * length) override { return item_reader_->GetRepLevels(data, length); }

            bool IsOrHasRepeatedChild() const final { return true; }

            Status LoadBatch(int64_t number_of_records) final { return item_reader_->LoadBatch(number_of_records); }

            virtual ::arrow::Result<std::shared_ptr<ChunkedArray>> AssembleArray(std::shared_ptr<ArrayData> data)
            {
            ...
            Status BuildArray(int64_t length_upper_bound, std::shared_ptr<ChunkedArray> * out) override
            {
                ...
                if (field_->nullable())
                {
                    ARROW_ASSIGN_OR_RAISE(validity_buffer, AllocateResizableBuffer(bit_util::BytesForBits(length_upper_bound), ctx_->pool));
                    validity_io.valid_bits = validity_buffer->mutable_data();
                }
                ARROW_ASSIGN_OR_RAISE(
                    std::shared_ptr<ResizableBuffer> offsets_buffer,
                    AllocateResizableBuffer(sizeof(IndexType) * std::max(int64_t{1}, length_upper_bound + 1), ctx_->pool));
                ...
                offset_data[0] = 0;
                BEGIN_PARQUET_CATCH_EXCEPTIONS
                ::parquet::internal::DefRepLevelsToList(def_levels, rep_levels, num_levels, level_info_, &validity_io, offset_data);
                END_PARQUET_CATCH_EXCEPTIONS
                ...
                RETURN_NOT_OK(offsets_buffer->Resize((validity_io.values_read + 1) * sizeof(IndexType)));
                if (validity_buffer != nullptr)
                {
                    RETURN_NOT_OK(validity_buffer->Resize(bit_util::BytesForBits(validity_io.values_read)));
                    validity_buffer->ZeroPadding();
                }
                ARROW_ASSIGN_OR_RAISE(std::shared_ptr<ArrayData> item_chunk, ChunksToSingle(**out));

                std::vector<std::shared_ptr<Buffer>> buffers{validity_io.null_count > 0 ? validity_buffer : nullptr, offsets_buffer};
                auto data = std::make_shared<ArrayData>(
                    field_->type(),
                    /*length=*/validity_io.values_read,
                    std::move(buffers),
        ...
        public:
            ...
            ::arrow::Result<std::shared_ptr<ChunkedArray>> AssembleArray(std::shared_ptr<ArrayData> data) final
            {
                ...
                for (int x = 1; x <= data->length; x++)
                {
                    ...
                    {
                        return Status::Invalid("Expected all lists to be of size=", type.list_size(), " but index ", x, " had size=", size);
                    }
                }
        ...
        public:
            explicit StructReader(
                ...
                : ctx_(std::move(ctx)), filtered_field_(std::move(filtered_field)), level_info_(level_info), children_(std::move(children))
            {
                ...
                auto result = std::find_if(
                    children_.begin(),
                    children_.end(),
                    [](const std::unique_ptr<ColumnReaderImpl> & child) { return !child->IsOrHasRepeatedChild(); });
        ...
        Status StructReader::BuildArray(int64_t length_upper_bound, std::shared_ptr<ChunkedArray> * out)
        {
            ...
            {
                ARROW_ASSIGN_OR_RAISE(null_bitmap, AllocateResizableBuffer(bit_util::BytesForBits(length_upper_bound), ctx_->pool));
            ...
            std::vector<std::shared_ptr<Buffer>> buffers{validity_io.null_count > 0 ? null_bitmap : nullptr};
            auto data = std::make_shared<ArrayData>(
                filtered_field_->type(),
                /*length=*/validity_io.values_read,
                std::move(buffers),
            ...
            std::unique_ptr<ColumnReaderImpl> * out)
        {
            ...
            {
                auto storage_field = arrow_field->WithType(checked_cast<const ExtensionType &>(*arrow_field->type()).storage_type());
            ...
            if (field.children.size() == 0)
            {
                ...
                type_id == ::arrow::Type::LIST || type_id == ::arrow::Type::MAP || type_id == ::arrow::Type::FIXED_SIZE_LIST
                || type_id == ::arrow::Type::LARGE_LIST)
            {
                ...
                {
                    return Status::Invalid("expected exactly one child field for: ", list_field->ToString());
                }
                const DataType & schema_child_type = *(list_field->type()->field(0)->type());
                if (type_id == ::arrow::Type::MAP)
                {
                    if (reader_child_type->num_fields() != 2
                        || !reader_child_type->field(0)->type()->Equals(*schema_child_type.field(0)->type()))
                    {
                    ...
                    else if (!reader_child_type->field(1)->type()->Equals(*schema_child_type.field(1)->type()))
                    {
                        list_field = list_field->WithType(std::make_shared<::arrow::MapType>(
                            reader_child_type->field(0), // field 0 is unchanged baed on previous if statement
                            reader_child_type->field(1)));
                    }
                    // Map types are list<struct<key, value>> so use ListReader
                    // for reconstruction.
                    out->reset(new ListReader<int32_t>(ctx, list_field, field.level_info, std::move(child_reader)));
                }
                ...
                else if (type_id == ::arrow::Type::LARGE_LIST)
                {
                    ...
                    out->reset(new ListReader<int64_t>(ctx, list_field, field.level_info, std::move(child_reader)));
                }
                else if (type_id == ::arrow::Type::FIXED_SIZE_LIST)
                {
                    if (!reader_child_type->Equals(schema_child_type))
                    {
                        auto & fixed_list_type = checked_cast<const ::arrow::FixedSizeListType &>(*list_field->type());
                        int32_t list_size = fixed_list_type.list_size();
                        list_field = list_field->WithType(::arrow::fixed_size_list(reader_child_type, list_size));
                    }

                    out->reset(new FixedSizeListReader(ctx, list_field, field.level_info, std::move(child_reader)));
                }
                else
                {
            ...
            else if (type_id == ::arrow::Type::STRUCT)
            {
                ...
                for (const auto & child : field.children)
                {
                    ...
                    const DataType & schema_child_type = *arrow_field->type()->field(arrow_field_idx++)->type();
                ...
                auto filtered_field
                    = ::arrow::field(arrow_field->name(), ::arrow::struct_(child_fields), arrow_field->nullable(), arrow_field->metadata());
        ...
        Status GetReader(const SchemaField & field, const std::shared_ptr<ReaderContext> & ctx, std::unique_ptr<ColumnReaderImpl> * out)
        {
    ...
    Status FileReaderImpl::GetRecordBatchReader(
        const std::vector<int> & row_groups, const std::vector<int> & column_indices, std::unique_ptr<RecordBatchReader> * out)
    {
        ...
        if (reader_properties_.pre_buffer())
        {
            ...
            BEGIN_PARQUET_CATCH_EXCEPTIONS
            reader_->PreBuffer(row_groups, column_indices, reader_properties_.io_context(), reader_properties_.cache_options());
            END_PARQUET_CATCH_EXCEPTIONS
        }
        ...
        if (readers.empty())
        {
            ...
            auto max_sized_batch = ::arrow::RecordBatch::Make(batch_schema, batch_size, ::arrow::ArrayVector{});
            ...
            *out = std::make_unique<RowGroupRecordBatchReader>(::arrow::MakeVectorIterator(std::move(batches)), std::move(batch_schema));
        ...
        ::arrow::Iterator<RecordBatchIterator> batches = ::arrow::MakeFunctionIterator(
            [readers, batch_schema, num_rows, this]() mutable -> ::arrow::Result<RecordBatchIterator>
            {
        ...
        *out = std::make_unique<RowGroupRecordBatchReader>(::arrow::MakeFlattenIterator(std::move(batches)), std::move(batch_schema));
    ...
    {
    public:
        using RecordBatchGenerator = ::arrow::AsyncGenerator<std::shared_ptr<::arrow::RecordBatch>>;

        explicit RowGroupGenerator(
            std::shared_ptr<FileReaderImpl> arrow_reader,
            ::arrow::internal::Executor * cpu_executor,
        ...
        ::arrow::Future<RecordBatchGenerator> operator()()
        {
            ...
            if (cpu_executor_)
                ready = cpu_executor_->TransferAlways(ready);
            return ready.Then(
    ...
    private:
            ...
            const std::vector<int> & column_indices)
        {
            ...
            return ::arrow::DeferNotOk(cpu_executor->Submit(ReadOneRowGroup, cpu_executor, self, row_group, column_indices));
        }
            ...
            const std::vector<int> & column_indices)
        {
            ...
            return self->DecodeRowGroups(self, {row_group}, column_indices, cpu_executor)
                .Then(
                    [batch_size](const std::shared_ptr<Table> & table) -> ::arrow::Result<RecordBatchGenerator>
                    {
    ...
    ::arrow::Result<::arrow::AsyncGenerator<std::shared_ptr<::arrow::RecordBatch>>> FileReaderImpl::GetRecordBatchGenerator(
        std::shared_ptr<FileReader> reader,
        const std::vector<int> row_group_indices,
        const std::vector<int> column_indices,
        ::arrow::internal::Executor * cpu_executor,
        int row_group_readahead)
    {
        ...
        {
            BEGIN_PARQUET_CATCH_EXCEPTIONS
            reader_->PreBuffer(row_group_indices, column_indices, reader_properties_.io_context(), reader_properties_.cache_options());
            END_PARQUET_CATCH_EXCEPTIONS
        }
        ::arrow::AsyncGenerator<RowGroupGenerator::RecordBatchGenerator> row_group_generator = RowGroupGenerator(
            ::arrow::internal::checked_pointer_cast<FileReaderImpl>(reader), cpu_executor, row_group_indices, column_indices);
        if (row_group_readahead > 0)
        {
            row_group_generator = ::arrow::MakeReadaheadGenerator(std::move(row_group_generator), row_group_readahead);
        }
        return ::arrow::MakeConcatenatedGenerator(std::move(row_group_generator));
    }

    Status FileReaderImpl::GetColumn(int i, FileColumnIteratorFactory iterator_factory, std::unique_ptr<ColumnReader> * out)
    {
        RETURN_NOT_OK(BoundsCheckColumn(i));
        auto ctx = std::make_shared<ReaderContext>();
        ctx->reader = reader_.get();
    ...
    Status FileReaderImpl::ReadRowGroups(
        const std::vector<int> & row_groups, const std::vector<int> & column_indices, std::shared_ptr<Table> * out)
    {
        RETURN_NOT_OK(BoundsCheck(row_groups, column_indices));
        ...
        {
            BEGIN_PARQUET_CATCH_EXCEPTIONS
            parquet_reader()->PreBuffer(row_groups, column_indices, reader_properties_.io_context(), reader_properties_.cache_options());
            END_PARQUET_CATCH_EXCEPTIONS
        }

        auto fut = DecodeRowGroups(
            /*self=*/nullptr,
            row_groups,
            column_indices,
        ...
        ::arrow::internal::Executor * cpu_executor)
    {
        ...
        if (!cpu_executor)
            cpu_executor = ::arrow::internal::GetCpuThreadPool();

        auto read_column
            = [row_groups, self, this](
                  size_t i, std::shared_ptr<ColumnReaderImpl> reader) -> ::arrow::Result<std::shared_ptr<::arrow::ChunkedArray>>
        {
        ...
        auto make_table =
            [result_schema, row_groups, self, this](const ::arrow::ChunkedArrayVector & columns) -> ::arrow::Result<std::shared_ptr<Table>>
        {
            int64_t num_rows = 0;
            if (!columns.empty())
            {
        ...
        return ::arrow::internal::OptionalParallelForAsync(reader_properties_.use_threads(), std::move(readers), read_column, cpu_executor)
    ...
    Status FileReader::GetRecordBatchReader(const std::vector<int> & row_group_indices, std::shared_ptr<RecordBatchReader> * out)
    {
    ...
    Status FileReader::GetRecordBatchReader(
        const std::vector<int> & row_group_indices, const std::vector<int> & column_indices, std::shared_ptr<RecordBatchReader> * out)
    {
        std::unique_ptr<RecordBatchReader> tmp;
    ...
    Status FileReader::Make(::arrow::MemoryPool * pool, std::unique_ptr<ParquetFileReader> reader, std::unique_ptr<FileReader> * out)
    {
        return Make(pool, std::move(reader), default_arrow_reader_properties(), out);
    }

    FileReaderBuilder::FileReaderBuilder() : pool_(::arrow::default_memory_pool()), properties_(default_arrow_reader_properties())
    {
    }

    Status FileReaderBuilder::Open(
        std::shared_ptr<::arrow::io::RandomAccessFile> file, const ReaderProperties & properties, std::shared_ptr<FileMetaData> metadata)
    {
        PARQUET_CATCH_NOT_OK(raw_reader_ = ParquetReader::Open(std::move(file), properties, std::move(metadata)));
        return Status::OK();
    }
    ...
    FileReaderBuilder * FileReaderBuilder::properties(const ArrowReaderProperties & arg_properties)
    {
    ...
    Status OpenFile(std::shared_ptr<::arrow::io::RandomAccessFile> file, MemoryPool * pool, std::unique_ptr<FileReader> * reader)
    {
