...
int LevelDecoder::SetData(Encoding::type encoding, int16_t max_level, int num_buffered_values, const uint8_t * data, int32_t data_size)
{
    ...
    switch (encoding)
    {
        case Encoding::RLE: {
            ...
            if (!rle_decoder_)
            {
                rle_decoder_.reset(new ::arrow::util::RleDecoder(decoder_data, num_bytes, bit_width_));
            }
            else
            {
        ...
        case Encoding::BIT_PACKED: {
            ...
            {
                throw ParquetException("Number of buffered values too large (corrupt data page?)");
            }
            ...
            {
                bit_packed_decoder_ = std::make_unique<::arrow::bit_util::BitReader>(data, num_bytes);
            }
            else
            {
...
void LevelDecoder::SetDataV2(int32_t num_bytes, int16_t max_level, int num_buffered_values, const uint8_t * data)
{
    max_level_ = max_level;
...
int LevelDecoder::Decode(int batch_size, int16_t * levels)
{
    ...
    if (num_decoded > 0)
    {
        ...
        {
            std::stringstream ss;
            ss << "Malformed levels. min: " << min_max.min << " max: " << min_max.max << " out of range.  Max Level: " << max_level_;
...
namespace
{
    ...
    private:
        void UpdateDecryption(const std::shared_ptr<Decryptor> & decryptor, int8_t module_type, std::string * page_aad);

        void InitDecryption();

        std::shared_ptr<Buffer>
        DecompressIfNeeded(std::shared_ptr<Buffer> page_buffer, int compressed_len, int uncompressed_len, int levels_byte_len = 0);
    ...
    void SerializedPageReader::UpdateDecryption(const std::shared_ptr<Decryptor> & decryptor, int8_t module_type, std::string * page_aad)
    {
        DCHECK(decryptor != nullptr);
        if (crypto_ctx_.start_decrypt_with_dictionary_page)
        {
            std::string aad = encryption::CreateModuleAad(
                decryptor->file_aad(), module_type, crypto_ctx_.row_group_ordinal, crypto_ctx_.column_ordinal, kNonPageOrdinal);
    ...
    {
        ThriftDeserializer deserializer(kDefaultThriftStringSizeLimit, kDefaultThriftContainerSizeLimit);
        ...
        while (seen_num_rows_ < total_num_rows_)
        {
            ...
            while (true)
            {
                ...
                try
                {
                    if (crypto_ctx_.meta_decryptor != nullptr)
                    {
                        UpdateDecryption(crypto_ctx_.meta_decryptor, encryption::kDictionaryPageHeader, &data_page_header_aad_);
                    }
                    deserializer.DeserializeMessage(
                        reinterpret_cast<const uint8_t *>(view.data()), &header_size, &current_page_header_, crypto_ctx_.meta_decryptor);
                    break;
                }
            ...
            if (crypto_ctx_.data_decryptor != nullptr)
            {
                UpdateDecryption(crypto_ctx_.data_decryptor, encryption::kDictionaryPage, &data_page_aad_);
            }
            ...
            {
                std::stringstream ss;
                ss << "Page was smaller (" << page_buffer->size() << ") than expected (" << compressed_len << ")";
            ...
            if (crypto_ctx_.data_decryptor != nullptr)
            {
                PARQUET_THROW_NOT_OK(decryption_buffer_->Resize(compressed_len - crypto_ctx_.data_decryptor->CiphertextSizeDelta(), false));
                compressed_len
                    = crypto_ctx_.data_decryptor->Decrypt(page_buffer->data(), compressed_len, decryption_buffer_->mutable_data());
            ...
            if (page_type == PageType::DICTIONARY_PAGE)
            {
                ...
                const format::DictionaryPageHeader & dict_header = current_page_header_.dictionary_page_header;
                ...
                page_buffer = DecompressIfNeeded(std::move(page_buffer), compressed_len, uncompressed_len);

                return std::make_shared<DictionaryPage>(
                    page_buffer, dict_header.num_values, LoadEnumSafe(&dict_header.encoding), is_sorted);
            }
            ...
            else if (page_type == PageType::DATA_PAGE_V2)
            {
                ...
                if (header.definition_levels_byte_length < 0 || header.repetition_levels_byte_length < 0)
                {
                ...
                // Uncompress if needed
                int levels_byte_len;
                if (AddWithOverflow(header.definition_levels_byte_length, header.repetition_levels_byte_length, &levels_byte_len))
                {
                ...
                if (is_compressed)
                {
                    page_buffer = DecompressIfNeeded(std::move(page_buffer), compressed_len, uncompressed_len, levels_byte_len);
                }
    ...
    std::shared_ptr<Buffer> SerializedPageReader::DecompressIfNeeded(
        std::shared_ptr<Buffer> page_buffer, int compressed_len, int uncompressed_len, int levels_byte_len)
    {
...
namespace
{
    ...
    protected:
        ...
        //
        // @returns: the number of values read into the out buffer
        int64_t ReadValuesSpaced(int64_t batch_size, T * out, int64_t null_count, uint8_t * valid_bits, int64_t valid_bits_offset)
        {
            return current_decoder_->DecodeSpaced(
                out, static_cast<int>(batch_size), static_cast<int>(null_count), valid_bits, valid_bits_offset);
        }
            ...
            while (true)
            {
                ...
                else if (current_page_->type() == PageType::DATA_PAGE)
                {
                    ...
                    const int64_t levels_byte_size
                        = InitializeLevelDecoders(*page, page->repetition_level_encoding(), page->definition_level_encoding());
        ...
        void ConfigureDictionary(const DictionaryPage * page)
        {
            ...
            if (page->encoding() == Encoding::PLAIN_DICTIONARY || page->encoding() == Encoding::PLAIN)
            {
            ...
            if (page->encoding() == Encoding::PLAIN_DICTIONARY || page->encoding() == Encoding::PLAIN)
            {
                ...
                decoders_[encoding] = std::unique_ptr<DecoderType>(dynamic_cast<DecoderType *>(decoder.release()));
            }
            else
            {
        ...
        int64_t
        InitializeLevelDecoders(const DataPage & page, Encoding::type repetition_level_encoding, Encoding::type definition_level_encoding)
        {
            // Read a data page.
            ...
            if (max_rep_level_ > 0)
            {
                int32_t rep_levels_bytes = repetition_level_decoder_.SetData(
                    repetition_level_encoding, max_rep_level_, static_cast<int>(num_buffered_values_), buffer, max_size);
            ...
            if (max_def_level_ > 0)
            {
                int32_t def_levels_bytes = definition_level_decoder_.SetData(
                    definition_level_encoding, max_def_level_, static_cast<int>(num_buffered_values_), buffer, max_size);
        ...
        int64_t InitializeLevelDecodersV2(const DataPageV2 & page)
        {
            ...
            const int64_t total_levels_length
                = static_cast<int64_t>(page.repetition_levels_byte_length()) + page.definition_levels_byte_length();
            ...
            if (max_rep_level_ > 0)
            {
                repetition_level_decoder_.SetDataV2(
                    page.repetition_levels_byte_length(), max_rep_level_, static_cast<int>(num_buffered_values_), buffer);
            ...
            if (max_def_level_ > 0)
            {
                definition_level_decoder_.SetDataV2(
                    page.definition_levels_byte_length(), max_def_level_, static_cast<int>(num_buffered_values_), buffer);
            }
    ...
    template <typename DType>
    class TypedColumnReaderImpl : public TypedColumnReader<DType>, public ColumnReaderImplBase<DType>
    {
    public:
        using T = typename DType::c_type;

        TypedColumnReaderImpl(const ColumnDescriptor * descr, std::unique_ptr<PageReader> pager, ::arrow::MemoryPool * pool)
        ...
        int64_t ReadBatch(int64_t batch_size, int16_t * def_levels, int16_t * rep_levels, T * values, int64_t * values_read) override;
    ...
    protected:
        void SetExposedEncoding(ExposedEncoding encoding) override { this->exposed_encoding_ = encoding; }
    ...
    private:
        ...
        void ReadLevels(int64_t batch_size, int16_t * def_levels, int16_t * rep_levels, int64_t * num_def_levels, int64_t * values_to_read)
        {
            batch_size = std::min(batch_size, this->num_buffered_values_ - this->num_decoded_values_);
        ...
        int32_t * dict_len)
    {
        ...
        {
            std::stringstream ss;
            ss << "Data page is not dictionary encoded. Encoding: " << EncodingToString(this->current_encoding_);
            throw ParquetException(ss.str());
        }
    ...
    int64_t TypedColumnReaderImpl<DType>::ReadBatch(
        int64_t batch_size, int16_t * def_levels, int16_t * rep_levels, T * values, int64_t * values_read)
    {
        ...
        if (this->max_def_level_ > 0)
        {
            ...
            else
            {
                ...
                *values_read = validity_io.values_read;
                ...
                total_values = this->ReadValuesSpaced(*values_read, values, static_cast<int>(null_count), valid_bits, valid_bits_offset);
            }
        ...
        while (HasNext() && rows_to_skip > 0)
        {
            ...
            else
            {
                ...
                std::shared_ptr<ResizableBuffer> scratch
                    = AllocateBuffer(this->pool_, batch_size * std::max<int>(sizeof(int16_t), value_size));
...
std::shared_ptr<ColumnReader> ColumnReader::Make(const ColumnDescriptor * descr, std::unique_ptr<PageReader> pager, MemoryPool * pool)
{
    switch (descr->physical_type())
    {
        case Type::BOOLEAN:
            return std::make_shared<TypedColumnReaderImpl<BooleanType>>(descr, std::move(pager), pool);
        case Type::INT32:
            return std::make_shared<TypedColumnReaderImpl<Int32Type>>(descr, std::move(pager), pool);
        case Type::INT64:
            return std::make_shared<TypedColumnReaderImpl<Int64Type>>(descr, std::move(pager), pool);
        case Type::INT96:
            return std::make_shared<TypedColumnReaderImpl<Int96Type>>(descr, std::move(pager), pool);
        case Type::FLOAT:
            return std::make_shared<TypedColumnReaderImpl<FloatType>>(descr, std::move(pager), pool);
        case Type::DOUBLE:
            return std::make_shared<TypedColumnReaderImpl<DoubleType>>(descr, std::move(pager), pool);
        case Type::BYTE_ARRAY:
            return std::make_shared<TypedColumnReaderImpl<ByteArrayType>>(descr, std::move(pager), pool);
        case Type::FIXED_LEN_BYTE_ARRAY:
            return std::make_shared<TypedColumnReaderImpl<FLBAType>>(descr, std::move(pager), pool);
        default:
...
namespace internal
{
    namespace
    {
        ...
        template <typename DType>
        class TypedRecordReader : public ColumnReaderImplBase<DType>, virtual public RecordReader
        {
        public:
            using T = typename DType::c_type;
            using BASE = ColumnReaderImplBase<DType>;
            TypedRecordReader(const ColumnDescriptor * descr, LevelInfo leaf_info, MemoryPool * pool) : BASE(descr, pool)
            {
            ...
            int64_t available_values_current_page() const { return this->num_buffered_values_ - this->num_decoded_values_; }
            ...
            void ReserveLevels(int64_t extra_levels)
            {
                if (this->max_def_level_ > 0)
                {
                    const int64_t new_levels_capacity = UpdateCapacity(levels_capacity_, levels_written_, extra_levels);
            ...
            {
                const int64_t new_values_capacity = UpdateCapacity(values_capacity_, values_written_, extra_values);
                if (new_values_capacity > values_capacity_)
                {
                    ...
                    if (uses_values_)
                    {
                        PARQUET_THROW_NOT_OK(values_->Resize(bytes_for_values(new_values_capacity), false));
                    }
                ...
                if (leaf_info_.HasNullableValues())
                {
                    ...
                    if (valid_bits_->size() < valid_bytes_new)
                    {
                        ...
                        memset(valid_bits_->mutable_data() + valid_bytes_old, 0, valid_bytes_new - valid_bytes_old);
                    }
                }
            }
            ...
            virtual void ReadValuesSpaced(int64_t values_with_nulls, int64_t null_count)
            {
                ...
                int64_t num_decoded = this->current_decoder_->DecodeSpaced(
                    ValuesHead<T>(), static_cast<int>(values_with_nulls), static_cast<int>(null_count), valid_bits, valid_bits_offset);
                DCHECK_EQ(num_decoded, values_with_nulls);
            }

            virtual void ReadValuesDense(int64_t values_to_read)
            {
                int64_t num_decoded = this->current_decoder_->Decode(ValuesHead<T>(), static_cast<int>(values_to_read));
            ...
            int64_t ReadRecordData(int64_t num_records)
            {
                ...
                const int64_t possible_num_values = std::max(num_records, levels_written_ - levels_position_);
                ...
                if (leaf_info_.HasNullableValues())
                {
                    ...
                    DefLevelsToBitmap(
                        def_levels() + start_levels_position, levels_position_ - start_levels_position, leaf_info_, &validity_io);
        ...
        class FLBARecordReader : public TypedRecordReader<FLBAType>, virtual public BinaryRecordReader
        {
        public:
            FLBARecordReader(const ColumnDescriptor * descr, LevelInfo leaf_info, ::arrow::MemoryPool * pool)
            ...
            void ReadValuesDense(int64_t values_to_read) override
            {
                ...
                int64_t num_decoded = this->current_decoder_->Decode(values, static_cast<int>(values_to_read));
                DCHECK_EQ(num_decoded, values_to_read);
            ...
            void ReadValuesSpaced(int64_t values_to_read, int64_t null_count) override
            {
                ...
                int64_t num_decoded = this->current_decoder_->DecodeSpaced(
                    values, static_cast<int>(values_to_read), static_cast<int>(null_count), valid_bits, valid_bits_offset);
                DCHECK_EQ(num_decoded, values_to_read);
        ...
        class ByteArrayChunkedRecordReader : public TypedRecordReader<ByteArrayType>, virtual public BinaryRecordReader
        {
        public:
            ByteArrayChunkedRecordReader(const ColumnDescriptor * descr, LevelInfo leaf_info, ::arrow::MemoryPool * pool)
            ...
            void ReadValuesDense(int64_t values_to_read) override
            {
                int64_t num_decoded = this->current_decoder_->DecodeArrowNonNull(static_cast<int>(values_to_read), &accumulator_);
                DCHECK_EQ(num_decoded, values_to_read);
                ResetValues();
            }
        ...
        public:
            ...
            {
                if (!internal_column)
                { // !internal_column happens at the last empty chunk
                    ::arrow::ArrayVector result = accumulator_.chunks;
            ...
            void ReadValuesDense(int64_t values_to_read) override
            {
                ...
                ::arrow::internal::BitmapWriter bitmap_writer(
                    const_cast<uint8_t *>(fake_array->data()->buffers[0]->data()), value_counter, values_to_read);

                createColumnIfNeeded();
                int64_t num_decoded = this->current_decoder_->DecodeCHNonNull(
                    static_cast<int>(values_to_read), column_chars_t_p, column_offsets_p, bitmap_writer);
                DCHECK_EQ(num_decoded, values_to_read);
                ResetValues();
        ...
        class ByteArrayDictionaryRecordReader : public TypedRecordReader<ByteArrayType>, virtual public DictionaryRecordReader
        {
        public:
            ByteArrayDictionaryRecordReader(const ColumnDescriptor * descr, LevelInfo leaf_info, ::arrow::MemoryPool * pool)
            ...
            void ReadValuesDense(int64_t values_to_read) override
            {
                ...
                {
                    num_decoded = this->current_decoder_->DecodeArrowNonNull(static_cast<int>(values_to_read), &builder_);
        ...
        std::shared_ptr<RecordReader>
        MakeByteArrayRecordReader(const ColumnDescriptor * descr, LevelInfo leaf_info, ::arrow::MemoryPool * pool, bool read_dictionary)
        {
            if (read_dictionary)
            {
    ...
    std::shared_ptr<RecordReader>
    RecordReader::Make(const ColumnDescriptor * descr, LevelInfo leaf_info, MemoryPool * pool, const bool read_dictionary)
    {
        switch (descr->physical_type())
        {
            case Type::BOOLEAN:
