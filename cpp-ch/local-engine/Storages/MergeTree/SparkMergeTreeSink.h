...
public:
    ...
    void collectStats(const std::deque<PartWithStats> & parts, const std::string & partition_dir) const
    {
        ...
        const std::string & partition = partition_dir.empty() ? NO_PARTITION_ID : partition_dir;
...
public:
    ...
    SinkPtr createSinkForPartition(const String & partition_id) override
    {
        ...
        return SparkMergeTreeSink::create(
            table, write_settings, context_->getGlobalContext(), empty_delta_stats_, {std::dynamic_pointer_cast<MergeTreeStats>(stats_)});
    }
};
