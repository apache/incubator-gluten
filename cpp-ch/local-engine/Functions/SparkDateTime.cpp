...
namespace DB
{
    ...
    namespace
    {
        ...
        public:
            ...
            DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName & arguments) const override
            {
                FunctionArgumentDescriptors mandatory_args{
                        {"time", &isStringOrDate32<IDataType>, nullptr, "String"},
            ...
            ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr & /*result_type*/, size_t input_rows_count) const override
            {
                ...
                ColumnUInt8::MutablePtr col_null_map = ColumnUInt8::create(input_rows_count, 0);

                if (isDate32(arguments[0].type)) {
                    const auto * col_date = checkAndGetColumn<ColumnDate32>(arguments[0].column.get());
                    auto col_res = ColumnDateTime::create(input_rows_count);
                    auto & res_data = col_res->getData();

                    for (size_t i = 0; i != input_rows_count; ++i) {
                        try {
                            auto daysFromEpoch = col_date->getData()[i];
                            auto time_zone_offset = time_zone.timezoneOffset(daysFromEpoch * 86400);
                            res_data[i] = static_cast<DataTypeDateTime::FieldType>(daysFromEpoch * 86400 - time_zone_offset);
                        } catch (...){
                            res_data[i] = 0;
        ...
        private:
            ...
            {
                static_assert(parse_syntax == ParseSyntax::Joda,
                        "parse syntax must be one Joda");
...
#undef ACTION_ARGS_WITH_BIND
        ...
        {
            static constexpr auto name = "sparkParseDateTimeInJodaSyntaxOrNull";
        };

        using SparkFunctionParseDateTimeInJodaSyntaxOrNull = FunctionParseDateTimeImpl<NameParseDateTimeInJodaSyntaxOrNull, ParseSyntax::Joda, ErrorHandling::Null>;
    }

    REGISTER_FUNCTION(SparkParseDateTime)
    {

        factory.registerFunction<SparkFunctionParseDateTimeInJodaSyntaxOrNull>();
    }
