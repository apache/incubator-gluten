        ...
        UInt32 scale)
    {
        ...
        auto call = [&](const auto & types) -> bool //-V657
        {
            ...
            if constexpr (IsDataTypeNumber<FromDataType>)
            {
                ...
                {
                    ToNativeType result;
                    bool convert_result = convertDecimalsFromIntegerImpl<FromFieldType, ToNativeType>(datas[i], result, precision_value);

                    if (convert_result)
                        vec_to[i] = static_cast<ToFieldType>(result);
                    else if constexpr (exception_mode == ConvertExceptionMode::Null)
                    {
                        ...
                        throw Exception(
                            ErrorCodes::ILLEGAL_COLUMN, "Cannot parse {} as {}", src_column.type->getName(), result_type->getName());
                }
