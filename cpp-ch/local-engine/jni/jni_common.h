...
public:
    ...
    void waitForZero()
    {
        ...
        cv.wait(lock, [this] {
          auto n = counter.load(std::memory_order_acquire);
          return n == 0;
        });
    }
protected:
...
#define LOCAL_ENGINE_JNI_DESTROY_METHOD_START \
    local_engine::JniEnvStatusWatcher::instance().inactive(); \
...
#define LOCAL_ENGINE_JNI_METHOD_START \
    ...
    { \
        ...
        { \
            ...
            { \
                std::cerr << "Call JNI method " << __FUNCTION__ << " when JNIEnv is not active!" << std::endl; \
                break; \
            }   
