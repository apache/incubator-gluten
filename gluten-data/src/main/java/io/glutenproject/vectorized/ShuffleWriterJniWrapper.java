...
public class ShuffleWriterJniWrapper extends JniInitialized {
   ...
   * @param preferEvict if true, write the partition buffer to disk once it is
   *     full
   * @param memoryPoolId
   * @return native shuffle writer instance id if created successfully.
   */
  public long make(NativePartitioning part, long offheapPerTask, int bufferSize,
                   String codec, String codecBackend,
   ...
   *
   * @param part contains the partitioning parameter needed by native shuffle
   *     writer
  ...
  public long makeForRSS(NativePartitioning part, long offheapPerTask,
                         int bufferSize, String codec,
                         int bufferCompressThreshold, int pushBufferMaxSize,
                         Object pusher, long memoryPoolId, long handle,
   ...
   */
  public native long nativeEvict(long shuffleWriterId, long size,
                                 boolean callBySelf) throws RuntimeException;

  /**
   * Split one record batch represented by bufAddrs and bufSizes into several
   * batches. The batch is split according to the first column as partition id.
   *
   * @param shuffleWriterId shuffle writer instance id
   ...
   * each partition's temporary file. And stop processing splitting
   *
   * @param shuffleWriterId shuffle writer instance id
   * @return SplitResult
   */
