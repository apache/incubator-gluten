 ...
 *   1. Decrease offheap and increase onheap memory size when whole stage fallback happened
 *   2. Increase executor heap memory if stage contains gluten operator and spark operator
 *      at the same time.
 *   Note: we don't support set resource profile for final stage now. Todo: will support it.
 */
@Experimental
