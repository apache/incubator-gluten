...
function process_script {
    ...
    sed -i '/^  run_and_time install_protobuf/a \ \ run_and_time install_awssdk' scripts/setup-ubuntu.sh
    sed -i 's/-mavx2 -mfma -mavx -mf16c -mlzcnt -std=c++17/-march=native -std=c++17 -mno-avx512f/g' scripts/setup-helper-functions.sh
}

function compile {
    scripts/setup-ubuntu.sh
    make release EXTRA_CMAKE_FLAGS="-DVELOX_ENABLE_PARQUET=ON -DVELOX_ENABLE_HDFS=ON -DVELOX_BUILD_TESTING=OFF -DVELOX_ENABLE_DUCKDB=OFF -DVELOX_BUILD_TEST_UTILS=ON -DVELOX_ENABLE_S3=ON -DVELOX_ENABLE_HIVE_CONNECTOR=ON"
}
