...
function process_setup_ubuntu {
  ...
  if [ $ENABLE_HDFS == "ON" ]; then
    sed -i '/^function install_fmt.*/ifunction install_kerberos {\n  wget https://web.mit.edu/kerberos/dist/krb5/1.20/krb5-1.20.tar.gz\n  tar -zxvf krb5-1.20.tar.gz\n  cd krb5-1.20/src\n  ./configure\n  make "-j$(nproc)"\n  sudo make install\n  sudo ldconfig\n}\n  ' scripts/setup-ubuntu.sh
    ...
    sed -i '/^  run_and_time install_fmt/a \ \ run_and_time install_kerberos' scripts/setup-ubuntu.sh
  fi
